...
game_menus = [
  ("start_game_0",menu_text_color(0xFF000000)|mnf_disable_all_keys,
    ...
    [
     ("continue",[],"Continue...",
	  ...
	  ## WINDYPLAINS+ ## - Allow rapid game entry.
	  ("quick_access",
		[
  ...
  ## WINDYPLAINS+ ## - Redirection of character creation to skip picking a starting region.
...
##      ]),
  ...
  ("reports",0,
   ...
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    [
## WINDYPLAINS+ ## - Consolidated reports menu.
		...
		("view_reference_materials",[],"Reference Material",
		...
		("view_modding_tools",
			...
			[(jump_to_menu, "mnu_modding_tools"),]),
	    
		("view_cheat_reports",[(ge,"$cheat_mode",1)],"Cheat Menu",
	...
	("modding_tools",0,
		...
		[
			...
			("reset_fief_names", [], "Reset Fief Names",
				[
					(call_script, "script_hub_initialize_original_center_names"),
				]),
			
			# ("relation_test", [], "Test relation Lady Safiya vs. Emir Lakhem",
				# [
					# (call_script, "script_troop_get_family_relation_to_troop", "trp_kingdom_6_lady_17", "trp_knight_6_9"),
					# (display_message, "@Lady vs. Guardian yields '{s11}'", gpu_debug),
					# (call_script, "script_troop_get_family_relation_to_troop", "trp_knight_6_9", "trp_kingdom_6_lady_17"),
					# (display_message, "@Guardian vs. Lady yields '{s11}'", gpu_debug),
				# ]),
				
			("cci_clear_commissions", [], "Clear all commissions.",
				[
					(call_script, "script_cci_clear_all_commission_entries"),
					(display_message, "@Commission list has been cleared.", gpu_debug),
				]),
				
			("test_stack_xp", [], "Test party stack xp amounts.",
				[
					...
					(try_for_range, ":stack_no", 0, ":stack_capacity"),
						...
						# Display the before xp amounts and number of upgrades available.
						(party_stack_get_experience, reg1, "p_main_party", ":stack_no"), # WSE
						(party_stack_get_num_upgradeable, reg2, "p_main_party", ":stack_no"), # WSE
						(display_message, "@DEBUG: {s21}'s stack has {reg1}xp and {reg2} upgrades available.", gpu_debug),
						
						# Add experience to each stack.
						(store_random_in_range, ":xp_gain", 0, 10000),
						(party_add_xp_to_stack, "p_main_party", ":stack_no", ":xp_gain"),
						(assign, reg1, ":xp_gain"),
						(display_message, "@DEBUG: {s21} had {reg1}xp added to its stack.", gpu_debug),
						
						# Display the after xp amounts and number of upgrades available.
						(party_stack_get_experience, reg1, "p_main_party", ":stack_no"), # WSE
						(party_stack_get_num_upgradeable, reg2, "p_main_party", ":stack_no"), # WSE
						(display_message, "@DEBUG: {s21}'s stack has {reg1}xp and {reg2} upgrades available.", gpu_debug),
			...
			# ("test_invalid_gear", [], "Test troops for invalid gear.",
				# [
					# (start_presentation, "prsnt_verify_troop_equipment"), 
				# ]),
				
			# ("test_gear", [], "Find best gear for type.",
				# [
					# (start_presentation, "prsnt_best_in_class"), 
				# ]),
			
			# ("test_comp_abil", [], "Display Abilities: Borcha",
				# [
					# (assign, ":troop_no", "trp_npc1"),
					# (try_for_range, ":requirement_slot", ce_requirements_begin, ce_requirements_end),
						# (store_sub, ":offset", ":requirement_slot", ce_requirements_begin),
						# (troop_get_slot, ":requirement", ":troop_no", ":requirement_slot"),
						# (call_script, "script_ce_store_troop_ability_string_to_s31", ":troop_no", ":requirement"),
						# (str_store_troop_name, s21, ":troop_no"),
						# (store_add, reg32, ":offset", 1),
						# (display_message, "@DEBUG: {s21} Requirement #{reg32} = {s31}", gpu_debug),
						# # Now show what they actually have.
						# (store_add, ":ability_slot", ":offset", abilities_begin),
						# (troop_get_slot, ":ability", ":troop_no", ":ability_slot"),
						# (call_script, "script_ce_store_troop_ability_string_to_s31", ":troop_no", ":ability"),
						# (str_store_troop_name, s21, ":troop_no"),
						# (store_add, reg32, ":offset", 1),
						# (display_message, "@DEBUG: {s21} Ability #{reg32} = {s31}", gpu_debug),
					# (try_end),
				# ]),
			
			# ("test_troop_ratings", [], "Compare troop prices and tiers.",
				# [
					
					# (try_for_range, ":troop_no", "trp_swadian_recruit", "trp_khergit_castle_guard"), # "trp_black_khergit_horseman"),
						# (troop_get_slot, reg41, ":troop_no", slot_troop_tier),
						# (troop_get_slot, reg42, ":troop_no", slot_troop_purchase_cost),
						# (call_script, "script_hub_determine_purchase_cost", ":troop_no"),
						# (troop_get_slot, reg43, ":troop_no", slot_troop_tier),
						# (troop_get_slot, reg44, ":troop_no", slot_troop_purchase_cost),
						# (str_store_troop_name, s31, ":troop_no"),
						# (str_length, ":string_length", s31),
						# (store_sub, ":spacing", 25, ":string_length"),
						# (val_max, ":spacing", 0),
						# (try_for_range, ":unused", 0, ":spacing"),
							# (str_store_string, s31, "@{s31} "),
						# (try_end),
						...
						# (try_begin),
							# (neq, reg41, reg43),
							# (str_store_string, s32, "@TIER {reg41} -> {reg43}"),
							# (assign, ":changed", 1),
							# (str_length, ":string_length", s32),
							# (store_sub, ":spacing", 18, ":string_length"),
						# (else_try),
							# (str_clear, s32),
							# (assign, ":spacing", 18),
						# (try_end),
						...
						# (try_begin),
							# (neq, reg42, reg44),
							# (str_store_string, s33, "@PRICE {reg42} -> {reg44}"),
							# (assign, ":changed", 1),
							# (str_length, ":string_length", s33),
							# (store_sub, ":spacing", 18, ":string_length"),
						# (else_try),
							# (str_clear, s32),
							# (assign, ":spacing", 18),
						...
						# (eq, ":changed", 1),
						# (display_message, "@DEBUG: {s31}{s32}{s33}.", gpu_debug),
					# (try_end),
				# ]),
			
			# ("action_test_looting", [], "Test party looting.",
				# [
					# ## Pick random lord party.
					# (try_for_range, ":unused", 0, 100),
						# (store_random_in_range, ":lord_no", active_npcs_begin, active_npcs_end),
					...
					# (display_message, "@DEBUG (loot): Looting {s31}.", gpu_green),
					# ## Calculate loot.
					# (call_script, "script_party_calculate_loot", ":party_no"),
					# ## auto-loot menu switch.
					# (call_script, "script_als_autoloot_redirect", "mnu_modding_tools"),
				# ]),
			
			# ("action_test_slots", [], "Test CENTER slots 470 & 499.",
				# [
					# (display_message, "@Testing slots for every center between slots 470 - 499.", gpu_green),
					# (try_for_range, ":center_no", centers_begin, centers_end),
						# (set_show_messages, 1), # Just incase.
						# # (try_begin),
							# # (neg|is_between, "$temp", villages_begin, villages_end),
							# # (assign, "$temp", villages_begin),
							# # (display_message, "@DEBUG: $temp exceeded village range.  Reset to first village.", gpu_red),
						# # (try_end),
						# (try_for_range, ":slot_no", 470, 499),
							# (neg|party_slot_eq, ":center_no", ":slot_no", 0), # If it isn't 0 I want to print it out.
							# (party_get_slot, reg21, ":center_no", ":slot_no"),
							# (assign, reg22, ":slot_no"),
							# (str_store_party_name, s21, ":center_no"),
							# (display_message, "@DEBUG: {s21} slot #{reg22} = {reg21}.", gpu_debug),
						# (try_end),
					# (try_end),
					# (display_message, "@End of records reached.", gpu_green),
					# #(val_add, "$temp", 1),
				# ]),
			
			# ("action_test_slots", [], "Test TROOP slots 460 - 520.",
				# [
					# (display_message, "@Testing slots for every troop between slots 460 - 520.", gpu_green),
					# (try_for_range, ":troop_no", 1, "trp_end_of_troops"),
						# (set_show_messages, 1), # Just incase.
						# (try_for_range, ":slot_no", 460, 521),
							# (neg|troop_slot_eq, ":troop_no", ":slot_no", 0), # If it isn't 0 I want to print it out.
							# (neg|is_between, ":troop_no", "trp_log_array_entry_type", "trp_quick_battle_troop_1"), # Player History Array
							# (troop_get_slot, reg21, ":troop_no", ":slot_no"),
							# (assign, reg22, ":slot_no"),
							# (str_store_troop_name, s21, ":troop_no"),
							# (assign, reg23, ":troop_no"),
							# (display_message, "@DEBUG: Troop #{reg23} \ {s21} - Slot #{reg22} = {reg21}.", gpu_debug),
						# (try_end),
					# (try_end),
					# (display_message, "@End of records reached.", gpu_green),
				# ]),
			
			# ("action_test_slots", [], "Test player mercenary checks.",
				# [
					# (try_begin),
						# (call_script, "script_cf_common_player_is_mercenary", 1),
						# (display_message, "@Player is a mercenary.", gpu_debug),
					# (else_try),
						# (call_script, "script_cf_common_player_is_mercenary", 0),
						# (display_message, "@Player is NOT a mercenary.", gpu_debug),
					# (else_try),
						# (display_message, "@ERROR - Unanalyzed condition.", gpu_error),
					# (try_end),
				# ]),
			
			("action_calc_encumbrance", [], "Calculate party encumbrance.",
				[
					(assign, ":cargo", 0),
					...
					(try_for_range, ":inv_slot", 10, ":inv_capacity"),
						...
						# (store_div, reg31, ":weight", 100),
						# (store_mod, reg32, ":weight", 100),
						# (store_div, reg33, ":cargo", 100),
						# (store_mod, reg34, ":cargo", 100),
						# (str_store_item_name, s31, ":item_no"),
						# (display_message, "@DEBUG: {s31} found weighting {reg31}.{reg32}.  Raises total cargo to {reg33}.{reg34}", gpu_debug),
					(try_end),
					# (store_div, reg21, ":cargo", 100),
					# (display_message, "@DEBUG: Final encumbrance is {reg21} lbs.", gpu_debug),
				]),
			
			("action_award_emblems", [], "Award 1 Emblem",
				[
...
### REPORTS - REFERENCE MENU - BEGIN ###
	("reference_reports",0,
		...
		[],
		[
			("action_view_all_items",[],"View All Troops",
				[
					(assign, "$vai_region_value", 0),
					(assign, "$vai_region_kingdom", kingdoms_begin),
			...
			("action_metrics",[],"View save game metric data",
				[
					(start_presentation, "prsnt_metrics_display"),
				]),
				
			("guide_tournaments", [(eq, "$g_wp_tpe_active", 1),], "Guide - Tournament Overhaul",	
				[
					(change_screen_return),
			...
			("guide_diplomacy", [], "Guide - Diplomacy System",	
				[
					(change_screen_return),
...
### REPORTS - KINGDOM MENU - BEGIN ###
	("kingdom_reports",0,
		...
		[],
		[
			("view_kingdom_management_report",
				[
					(assign, ":continue", 0),
					(try_begin),
						(call_script, "script_cf_qus_player_is_king", 1), # Player must be a king to see this.
						(assign, ":continue", 1),
					(else_try),
						(call_script, "script_cf_qus_player_is_vassal", 1), # Player may see this as a vassal, but they won't be able to edit anything.
						(assign, ":continue", 1),
					(try_end),
					(try_begin),
						(eq, ":continue", 0),
						(disable_menu_option),
					(try_end),
				],"View kingdom management report.",
				[
					## Clean out KMS_OBJECTS.
					(try_for_range, ":slot", 0, 400),
						(troop_set_slot, KMS_OBJECTS, ":slot", 0),
					(try_end),
					
					# Initialize information.
					(call_script, "script_diplomacy_sync_faction_data", "$players_kingdom", STORE_TO_PRESENTATION),
			...
			("kmt_lord_holdings",[],"Review lord holdings.", 
				[
					(store_sub, ":value", "$players_kingdom", kingdoms_begin),
					(try_begin),
						(faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_inactive),
						(val_sub, ":value", 1), # This is done because an inactive player faction isn't displayed so things are shifted off by 1.
					(try_end),
...
### REPORTS - PERSONAL MENU - BEGIN ###
	("personal_reports",0,
		...
		[
			...
			("view_character_abilities",[],"View character abilities.",
				[
			...
			("view_weekly_budget_report",[],"View weekly budget.",
				[
			...
			("view_trade_ledger",
				[
					...
					(ge, reg0, 1),
					(try_begin),
						(neg|troop_slot_ge, "trp_player", slot_troop_trade_ledger, 1),
						(disable_menu_option),
						(str_store_string, s1, "@ (assess prices to enable)"),
					(else_try),
						(str_clear, s1),
					(try_end),
				],"View your trade ledger.{s1}",
				[
					(assign, reg60, 0), #begin in Default view
					(start_presentation, "prsnt_trade_ledger_basic"),
				]),
			
			("view_tournament_report",[(eq, "$g_wp_tpe_active", 1),],"View tournament history.",
				[
					(change_screen_return),
					(assign, "$tournament_town", "$current_town"),
					(assign, "$tournament_log_mode", TPE_LOG_MODE_LIST_ONLY_SELECTED_CITY),
					(start_presentation, "prsnt_tpe_hall_of_fame"),
				]),
			
			("player_emblem_options",[],"Player emblem options.",
				[
					(change_screen_return),
...
### REPORTS - PARTY MENU - BEGIN ###
    ("party_reports",0,
		...
		[],
		[
			("view_party_size_report",[],"View force size report.",
				[(jump_to_menu, "mnu_party_size_report"),]),

			# Native Morale System
			("view_morale_report",[(neq, "$diplomacy_use_alt_morale", 1),],"View force morale report.",
				[(jump_to_menu, "mnu_morale_report"),]),
			
			# Alternate Morale System
			# ("view_morale_report_alt",[(eq, "$diplomacy_use_alt_morale", 1),],"View force morale report.",
				# [(jump_to_menu, "mnu_diplomacy_alt_morale_report"),]),
				
			("view_morale_ui",[(eq, "$diplomacy_use_alt_morale", 1),],"View party morale report.",
				[(start_presentation, "prsnt_party_morale"),]),
			
			("view_companion_relationship_report", 
				[
					(assign, reg11, 0),
					(try_for_range, ":troop_no", companions_begin, companions_end),
				...
				[
					(assign, "$return_menu", "mnu_party_reports"),
			...
			("view_stealth_report",[],"View party stealth chance.",
				[(jump_to_menu, "mnu_stealth_report"),]),
			
			## PreBattle Orders & Deployment
			("action_prebattle_custom_divisions", [], "Manage Split Troop Assignments.",
...
##        (troop_add_item, "trp_player","itm_furs",0),
      ...
      (else_try),
        ...
        (troop_raise_proficiency, "trp_player",wpt_archery,30),
        (troop_raise_proficiency, "trp_player",wpt_throwing,10),
        (troop_add_item, "trp_player","itm_plain_cavalry_shield",imod_battered),
...
#             (start_presentation, "prsnt_custom_banner"),
  ...
  ("morale_report",0,
   ...
   [
    ...
    [
      ("continue",[],"Continue...",
      [
		## WINDYPLAINS+ ## - Morale report returns to new "party reports" menu.
		#(jump_to_menu, "mnu_reports"),
		(jump_to_menu, "mnu_party_reports"),
		## WINDYPLAINS- ##
      ]),
  ...
  ("companion_report",0,
   ...
   (try_for_range, ":companion", companions_begin, companions_end),
		...
		(try_begin),
			...
			## WINDYPLAINS+ ## - Add in option to allow cheat mode to see where companions are even if they're not assigned.
			...
			(try_begin),
			## WINDYPLAINS+ ## - Companion report tells you where inactive companions are staying. (Cheat Mode)
				### COMPANION IS IN THE STANDARD INACTIVE ROTATION ###
				(troop_slot_eq, ":companion", slot_troop_occupation, slto_inactive),
				(troop_slot_eq, ":companion", slot_troop_prisoner_of_party, -1),
				(troop_get_slot, ":center_no", ":companion", slot_troop_cur_center),
				(str_store_party_name, s21, ":center_no"),
				(str_store_string, s8, "@Staying in {s21}."),
				(str_store_string, s5, "@INACTIVE"),
			(else_try),
				### COMPANION IS A PRISONER ###
				(troop_slot_eq, ":companion", slot_troop_occupation, slto_inactive),
				(troop_slot_ge, ":companion", slot_troop_prisoner_of_party, 0),
				(troop_get_slot, ":prisoner_of_party", ":companion", slot_troop_prisoner_of_party),
				(party_is_active, ":prisoner_of_party"),
				(str_store_party_name, s21, ":prisoner_of_party"),
				(str_store_string, s8, "@Captive of {s21}."),
				(str_store_string, s5, "@INACTIVE"),
			(else_try),
			## WINDYPLAINS- ##
			...
			## WINDYPLAINS+ ## - Enhanced Diplomacy - Update companion report to list advisers.
			(else_try),
				(troop_slot_ge, ":companion", slot_troop_advisor_role, 1),
				(troop_get_slot, ":station", ":companion", slot_troop_advisor_station),
				(troop_get_slot, ":position", ":companion", slot_troop_advisor_role),
				(is_between, ":position", advisors_begin, advisors_end),
				# Name the location.
				(str_store_party_name, s9, ":station"),
				#(str_store_string, s5, "str_in_your_court_at_s9"),
				(str_store_string, s5, "@{s9}"),
				# Name the position.
				(store_sub, ":advisor_no", ":position", advisors_begin),
				(store_add, ":string_no", ":advisor_no", "str_diplomacy_advisor_steward"),
				(str_store_string, s21, ":string_no"),
				(str_store_string, s8, "@Serving as {s21}"),
			## WINDYPLAINS- ##
			## WINDYPLAINS+ ## - Quest Utilities - Update companion report to describe troops left on assignment somewhere.
			(else_try),
				(troop_slot_eq, ":companion", slot_troop_current_mission, npc_mission_remaining_in_quest_location),
				# Name the location.
				(troop_get_slot, ":center_no", ":companion", slot_troop_cur_center),
				(str_store_party_name, s5, ":center_no"),
				# Name the position.
				(str_store_string, s8, "@On assignment"),
			## WINDYPLAINS- ##
			(else_try),	
  ...
  ("character_report",0,
   ...
   [(try_begin),
    ...
    (try_begin),
      ...
      ## WINDYPLAINS+ ## - Character report displays if you're a mercenary or not.
	  (try_begin),
		(call_script, "script_cf_common_player_is_mercenary", 0),
		(try_begin),
	  ...
	  (else_try),
		...
		# (assign, reg31, ":current_day"),
		# (assign, reg32, "$mercenary_service_next_renew_day"),
		# (display_message, "@DEBUG: Current Day = {reg31}, Renew Day = {reg32}", gpu_debug),
		(store_sub, reg21, "$mercenary_service_next_renew_day", ":current_day"),
		(str_store_string, s9, "@You are a mercenary in service of the {s8}.^You have {reg21} days left on your contract.^{s9}"),
	  (try_end),
	  ## WINDYPLAINS- ##
	...
	("continue",[(eq,"$cheat_mode",1),
       ...
       [
	   (call_script, "script_change_player_relation_with_troop", "$g_talk_troop", 10, 1),
	   (jump_to_menu, "mnu_character_report"),
       ]
       ),
	...
	("continue",[],"Continue...",
       [
		   ## WINDYPLAINS+ ## - Character report returns to new "personal reports" menu.
		   #(jump_to_menu, "mnu_reports"),
		   (jump_to_menu, "mnu_personal_reports"),
		   ## WINDYPLAINS- ##
        ]
  ...
  ("party_size_report",0,
   ...
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    ## WINDYPLAINS+ ## - Increase Charisma & Leadership contributions to player party size.
    (store_skill_level, ":leadership", "skl_leadership", "trp_player"),
    (val_mul, ":leadership", 8), # (val_mul, ":leadership", 5),
    (store_attribute_level, ":charisma", "trp_player", ca_charisma),
	(val_mul, ":charisma", 2), # Not Native
	...
	(try_begin),
		...
		(val_add, ":leadership", ":rallying_bonus"),
	(try_end),
	...
	# Account for player titles such as king or marshal.
	(assign, ":titles", 0),
	...
	# Account for every castle owned by the player.
	(assign, ":fiefs", 0),
	(try_for_range, ":center_no", centers_begin, centers_end),
	  ...
	  (try_begin),
		...
		# (assign, reg31, 10),
		# (assign, reg32, ":fiefs"),
		# (display_message, "@Party Size: Town of {s31} improves size by +{reg31}.  Total = {reg32}", gpu_debug),
	  (else_try),
		...
		# (assign, reg31, 16),
		# (assign, reg32, ":fiefs"),
		# (display_message, "@Party Size: Castle of {s31} improves size by +{reg31}.  Total = {reg32}", gpu_debug),
	  (else_try),
		...
		# (assign, reg31, 4),
		# (assign, reg32, ":fiefs"),
		# (display_message, "@Party Size: Village of {s31} improves size by +{reg31}.  Total = {reg32}", gpu_debug),
	  (try_end),
	(try_end),
	
	# Account for player's faction policies.
	(try_begin),
		(call_script, "script_cf_common_player_is_vassal_or_greater", 1),
		(faction_get_slot, ":policies", "$players_kingdom", slot_faction_army_size_adjust),
	(else_try),
		(assign, ":policies", 0),
	(try_end),
	(try_begin),
	...
	(assign, reg4, ":fiefs"),
	(assign, reg6, ":titles"),
	(assign, reg7, ":policies"),
    (str_store_string, s1, "@Current party size limit is {reg5}.^Current party size modifiers are:^^Base size:  +30^Leadership: {s2}{reg1}^Charisma: {s3}{reg2}^Renown: {s4}{reg3}^Lands: +{reg4}^Titles: +{reg6}^Policies: {s7}{reg7}^TOTAL:  {reg5}"),
    ## WINDYPLAINS- ##
	],
    [
      ("continue",[],"Continue...",
       [
			## WINDYPLAINS+ ## - Party size report returns to new "party reports" menu.
		   #(jump_to_menu, "mnu_reports"),
		   (jump_to_menu, "mnu_party_reports"),
		   ## WINDYPLAINS- ##
        ]
   ...
   [
    ...
    [
      ...
      ("camp_action",[],"Take an action.",
	   ...
	   ## WINDYPLAINS+ ##
	   # ("bts_options",[(ge, BETA_TESTING_MODE, 1),],"Access Beta Testing Options", [(start_presentation, "prsnt_bts_options")]),\
	   # ("bts_item_balancing",[(ge, BETA_TESTING_MODE, 2),],"Access Item Testing Options", [(start_presentation, "prsnt_bts_item_balancing")]),
	   ## WINDYPLAINS- ##
  ...
  ("cheat_find_item",0,
   ...
   [
	 ...
	 (val_sub, reg6, 1),
	 ## WINDYPLAINS+ ## - CHEAT - Display item names in the find item cheat.
	 (str_clear, s30),
	 (try_for_range, ":item_no", reg5, reg6),
		(str_store_item_name, s31, ":item_no"),
		(assign, reg31, ":item_no"),
		(str_store_string, s30, "@{s30}^#{reg31} - {s31}"),
	 (try_end),
	 ## WINDYPLAINS- ##
     ],
    [
      ("cheat_find_item_next_range",[], "{!}Move to next item range.",
       [
		...
		## WINDYPLAINS+ ## - Previous item range + IMOD option.  Credit: Somebody.
    ...
    [
      ...
      ("camp_recruit_prisoners",
       [(troops_can_join, 1),
        ...
        (try_end),
		## WINDYPLAINS+ ## - Don't ask to recruit prisoners to the party if you don't have any.
      ...
      ("action_rename_kingdom",
	   ...
	   ## WINDYPLAINS+ ## - Faction color changing menu option.
	   ("action_recolor_kingdom",
       [
         # (eq, "$players_kingdom_name_set", 1),
         # (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
         # (faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
         ],"Change the faction colors.",
       [(start_presentation, "prsnt_change_all_factions_color"),
        ]
       ),
	   ## WINDYPLAINS- ##
	   
		## WINDYPLAINS+ ## - Removed modifying your banner as a cheat and made it a normal feature.
	  ...
	  ("action_modify_banner",[
	   ...
	   ## WINDYPLAINS- ##
		...
		## WINDYPLAINS+ ## - 
		("action_banner_adjustments",[],"Modify banner background colors",
			[
				(try_begin),
					(troop_get_slot, ":banner_no", GPU_OBJECTS, gpu_val_current_banner),
					(neg|is_between, ":banner_no", banner_meshes_begin, banner_meshes_end_minus_one),
					(troop_set_slot, GPU_OBJECTS, gpu_val_current_banner, banner_meshes_begin),
					(troop_set_slot, GPU_OBJECTS, gpu_val_banner_background, gpu_white),
				(try_end),
    ...
    [
      ("camp_recruit_prisoners_accept",[(gt, "$g_prisoner_recruit_troop_id", 0)],"Take them.",
       [(remove_troops_from_prisoners, "$g_prisoner_recruit_troop_id", "$g_prisoner_recruit_size"),
        ...
        (call_script, "script_change_player_party_morale", ":morale_change", PMR_RECRUITED_PRISONERS),
        (jump_to_menu, "mnu_camp"),
        ]
       ),
  ...
  ("camp_action_read_book",0,
   ...
   [],
    [
      ## WINDYPLAINS+ ##
		("action_instant_finish_book",
			[
				(call_script, "script_emblem_get_current_quantity"), # emblem_scripts.py
				(ge, reg1, EMBLEM_COST_FINISH_BOOK_PLAYER),
				(gt, "$g_player_reading_book", 0),
				(player_has_item, "$g_player_reading_book"),
				(neg|item_slot_ge, "$g_player_reading_book", slot_item_book_reading_progress, 995),
				(assign, reg21, EMBLEM_COST_FINISH_BOOK_PLAYER),
				(store_sub, reg22, reg21, 1),
				(str_store_item_name, s21, "$g_player_reading_book"),
			],"Finish '{s21}'. ({reg21} Emblem{reg22?s:})",
			[
	  ...
	  ("action_read_book_1",[(player_has_item, "itm_book_tactics"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_tactics"),
							 (neq, "$g_player_reading_book", "itm_book_tactics"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_2",[(player_has_item, "itm_book_persuasion"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_persuasion"),
							 (neq, "$g_player_reading_book", "itm_book_persuasion"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_3",[(player_has_item, "itm_book_leadership"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_leadership"),
							 (neq, "$g_player_reading_book", "itm_book_leadership"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_4",[(player_has_item, "itm_book_intelligence"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_intelligence"),
							 (neq, "$g_player_reading_book", "itm_book_intelligence"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_5",[(player_has_item, "itm_book_trade"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_trade"),
							 (neq, "$g_player_reading_book", "itm_book_trade"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_6",[(player_has_item, "itm_book_weapon_mastery"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_weapon_mastery"),
							 (neq, "$g_player_reading_book", "itm_book_weapon_mastery"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
      ...
      ("action_read_book_7",[(player_has_item, "itm_book_engineering"),
							 ...
							 ## WINDYPLAINS+ ## - More information about a book's progress.
							 (call_script, "script_report_player_reading_progress", "itm_book_engineering"),
							 (neq, "$g_player_reading_book", "itm_book_engineering"),
							 ## WINDYPLAINS- ##
                             ],"{s1}.",
	  ...
	  ## WINDYPLAINS+ ## - Add in books to read.
	  ("action_read_book_8",
		[
			(player_has_item, "itm_book_tracking"),
			(item_slot_eq, "itm_book_tracking", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_tracking"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_tracking"),
			(neq, "$g_player_reading_book", "itm_book_tracking"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
	  ...
	  ("action_read_book_9",
		[
			(player_has_item, "itm_book_training"),
			(item_slot_eq, "itm_book_training", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_training"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_training"),
			(neq, "$g_player_reading_book", "itm_book_training"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_training"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  
	  ("action_read_book_10",
		[
			(player_has_item, "itm_book_firstaid"),
			(item_slot_eq, "itm_book_firstaid", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_firstaid"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_firstaid"),
			(neq, "$g_player_reading_book", "itm_book_firstaid"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_firstaid"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  
	  ("action_read_book_11",
		[
			(player_has_item, "itm_book_prison_management"),
			(item_slot_eq, "itm_book_prison_management", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_prison_management"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_prison_management"),
			(neq, "$g_player_reading_book", "itm_book_prison_management"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_prison_management"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  
	  ("action_read_book_12",
		[
			(player_has_item, "itm_book_charisma"),
			(item_slot_eq, "itm_book_charisma", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_charisma"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_charisma"),
			(neq, "$g_player_reading_book", "itm_book_charisma"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
	  ...
	  ("action_read_book_13",
		[
			(player_has_item, "itm_book_repair_bonus"),
			(item_slot_eq, "itm_book_repair_bonus", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_repair_bonus"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_repair_bonus"),
			(neq, "$g_player_reading_book", "itm_book_repair_bonus"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_repair_bonus"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
		
	  ("action_read_book_14",
		[
			(player_has_item, "itm_book_tax_reduction"),
			(item_slot_eq, "itm_book_tax_reduction", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_tax_reduction"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_tax_reduction"),
			(neq, "$g_player_reading_book", "itm_book_tax_reduction"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_tax_reduction"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  
	  ("action_read_book_15",
		[
			(player_has_item, "itm_book_cunning_rule"),
			(item_slot_eq, "itm_book_cunning_rule", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_cunning_rule"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_cunning_rule"),
			(neq, "$g_player_reading_book", "itm_book_cunning_rule"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_cunning_rule"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  
	  ("action_read_book_16",
		[
			(player_has_item, "itm_book_prosperity_1"),
			(item_slot_eq, "itm_book_prosperity_1", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_prosperity_1"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_prosperity_1"),
			(neq, "$g_player_reading_book", "itm_book_prosperity_1"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_prosperity_1"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
		
	  ("action_read_book_17",
		[
			(player_has_item, "itm_book_prof75_sword"),
			(item_slot_eq, "itm_book_prof75_sword", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_prof75_sword"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_prof75_sword"),
			(neq, "$g_player_reading_book", "itm_book_prof75_sword"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_prof75_sword"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
		
	  ("action_read_book_18",
		[
			(player_has_item, "itm_book_prof75_crossbow"),
			(item_slot_eq, "itm_book_prof75_crossbow", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_prof75_crossbow"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_prof75_crossbow"),
			(neq, "$g_player_reading_book", "itm_book_prof75_crossbow"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_prof75_crossbow"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
		
	  ("action_read_book_19",
		[
			(player_has_item, "itm_book_escape_chance"),
			(item_slot_eq, "itm_book_escape_chance", slot_item_book_read, 0),
			(str_store_item_name, s1, "itm_book_escape_chance"),
			## WINDYPLAINS+ ## - More information about a book's progress.
			(call_script, "script_report_player_reading_progress", "itm_book_escape_chance"),
			(neq, "$g_player_reading_book", "itm_book_escape_chance"),
			## WINDYPLAINS- ##
		],"{s1}.",
		[
			(assign, "$temp", "itm_book_escape_chance"),
			(jump_to_menu, "mnu_camp_action_read_book_start"),
		]),
	  ## WINDYPLAINS- ## 
      ("camp_action_4",[],"Back to camp menu.",
 ...
 Try as you might, it soon gives you a headache, and you're forced to give up the attempt."),
    (try_end),],
    [
		## WINDYPLAINS+ ## - CHEAT - Allow instant completion of reading a book while in cheat mode.
		("continue",
			[
				(item_slot_eq, "$temp", slot_item_book_read, 0),
				(ge, "$cheat_mode", 1),
			]
			,"CHEAT - Instantly complete reading of this book.",
			[
				(item_set_slot, "$temp", slot_item_book_read, 1),
				(str_store_item_name, s2, "$temp"),
				(display_message, "@You have completed reading {s21}.", gpu_green),
				(assign, "$g_player_reading_book", 0),
				(jump_to_menu, "mnu_camp_action_read_book_start"),
			]),
		## WINDYPLAINS- ##
 ...
 all your money, and ride off into the sunset....",
    ...
    [
      ("cattle_drive_away",[],"Drive the cattle onward.",
       [
		...
		## WINDYPLAINS+ ## - Change AI of cattle herds to follow you.
  ...
  ("cattle_herd_kill",0,
   ...
   "none",
   [
		## WINDYPLAINS+ ## - CMS - Altered to allow slaughtering some of the cattle herd for food to be picked up by the storekeeper.
		(try_begin),
  ...
  ("cattle_herd_kill_end",0,
   ...
   "none",
    ## WINDYPLAINS+ ## - Slaughtering cattle redirects to autoloot system.
	[
		(change_screen_return),
		(call_script, "script_als_autoloot_redirect", "mnu_cattle_herd_kill"),
	],
	## WINDYPLAINS- ##
    [
      ]
    ...
    [      
        ...
        (try_begin),
          ...
          (else_try),
			...
			## WINDYPLAINS+ ## - Prevent villagers aiding the player in defence of a village.
			(eq, "$disable_village_allies", 0),
			## WINDYPLAINS- ##
 ...
 but you'll have to leave {reg4} soldiers behind to stop the enemy from giving chase.",
    ...
    [
	  ...
	  [
		...
		(try_end),
		(call_script, "script_change_player_party_morale", -20, PMR_SACRIFICED_MEN),
  ...
  (
    ...
    "{s11}^^Your Casualties:{s8}{s10}^^Enemy Casualties:{s9}^^Kill count:{s12}", ## WINDYPLAINS+ ##
    "none",
    [
...
#NPC companion changes end
     ...
     (str_store_string_reg, s9, s0),
	 ## WINDYPLAINS+ ## - Add kill & wounded count info to menu "battle_debrief".  Credit: Custom Commander.
     ...
     (try_end),
	 ## WINDYPLAINS+ ## - Increase battle-weariness after each combat.
	 (store_current_hours, "$morale_time_last_battle"),
	 (call_script, "script_diplomacy_get_battle_weariness_factor", WEARINESS_PENALTY, 21),
	 (val_add, "$morale_battle_weary", reg21),
	 ...
	 (try_begin),
		...
		(assign, reg31, reg21),
		(display_message, "@DEBUG (BTM-2): Battle weariness ({reg33}) ({reg31})", gpu_debug),
	 (try_end),
	 ## WINDYPLAINS- ##
     ],
    [
  ...
  (
    ...
    [
        ...
        (try_begin),
          ...
          (try_begin),
            ...
            (try_begin),
              ...
              (val_div, ":rel_boost", 3),
            (try_end),
            (call_script,"script_change_player_relation_with_troop", ":ally_leader", ":rel_boost", 1),
			## WINDYPLAINS+ ## - Give relation boost with center for helping defend against a siege.
			(try_begin),
				(eq, "$g_encountered_party", "$current_town"), # You helped defend against a siege.
        ...
        (else_try),          
		  ...
		  ## WINDYPLAINS+ ## - Autoloot - Redirect to menu vs. looting screen.
		  # Original code section by Fisheye and from Custom Commander mod.
          # Autoloot: Instead of just displaying the loot screen, we display a loot management menu instead
		  (call_script, "script_als_autoloot_redirect", "mnu_total_victory"),
          ## WINDYPLAINS- ##
        (else_try),
          #finished all
          (try_begin),
          ...
          (else_try),            
            (try_begin), #my kingdom
              ...
              (else_try), #player took a walled center for player's kingdom
				...
				(assign, ":best_troop", "trp_n_swadian_crossbowman"),
...
##            (eq, ":random_attribute", ca_intelligence),
  ...
  (
    "pre_join",0,
    "You come across a battle between {s2} and {s1}. You decide to...{s24}{s25}",
    "none",
    [
        (str_store_party_name, 1,"$g_encountered_party"),
        (str_store_party_name, 2,"$g_encountered_party_2"),
		
		## DISPLAY HOST: $g_encountered_party
		(assign, ":host_party", "$g_encountered_party"),
		(try_begin),
			(party_slot_eq, ":host_party", slot_party_type, spt_kingdom_hero_party),
			(store_faction_of_party, ":host_faction", ":host_party"),
			(str_store_party_name, s31, ":host_party"),
			(str_store_string, s24, "@^^{s31}'s Host:"),
			(party_get_position, pos1, ":host_party"),
			(call_script, "script_party_count_fit_for_battle", ":host_party"),
			(assign, ":total_count", reg0),
			(assign, reg31, ":total_count"),
			(store_sub, reg32, reg31, 1),
			(str_store_party_name, s31, ":host_party"),
			(str_store_string, s24, "@{s24}^{s31} has {reg31} soldier{reg32?s:}."),
			(try_for_parties, ":party_no"),
				(party_is_active, ":party_no"),
				(neq, ":party_no", ":host_party"),
				(neq, ":party_no", "p_main_party"),
				(store_distance_to_party_from_party, ":dist", ":party_no", ":host_party"),
				(lt, ":dist", 1),
				(this_or_next|party_slot_eq, ":party_no", slot_party_type, spt_kingdom_hero_party),
				(this_or_next|party_slot_eq, ":party_no", slot_party_type, spt_deserter),
				(party_slot_eq, ":party_no", slot_party_type, spt_raider),
				
				(store_faction_of_party, ":party_faction", ":party_no"),
				(store_relation, ":relation", ":party_faction", ":host_faction"),
				(ge, ":relation", 0),
			...
			(try_end),
			(party_stack_get_troop_id, ":troop_no", ":host_party", 0),
			(troop_get_type, reg23, ":troop_no"),
			(assign, reg21, ":total_count"),
			(store_sub, reg22, reg21, 1),
			(str_store_string, s24, "@{s24}^{reg23?Her:His} host contains {reg21} soldier{reg22?s:}."),
		(else_try),
			(str_clear, s24),
		(try_end),
		
		## DISPLAY HOST: $g_encountered_party_2
		(assign, ":host_party", "$g_encountered_party_2"),
		(try_begin),
			(party_slot_eq, ":host_party", slot_party_type, spt_kingdom_hero_party),
			(store_faction_of_party, ":host_faction", ":host_party"),
			(str_store_party_name, s31, ":host_party"),
			(str_store_string, s25, "@^^{s31}'s Host:"),
			(party_get_position, pos1, ":host_party"),
			(call_script, "script_party_count_fit_for_battle", ":host_party"),
			(assign, ":total_count", reg0),
			(assign, reg31, ":total_count"),
			(store_sub, reg32, reg31, 1),
			(str_store_party_name, s31, ":host_party"),
			(str_store_string, s25, "@{s25}^{s31} has {reg31} soldier{reg32?s:}."),
			(try_for_parties, ":party_no"),
				(party_is_active, ":party_no"),
				(neq, ":party_no", ":host_party"),
				(neq, ":party_no", "p_main_party"),
			...
			(assign, reg21, ":total_count"),
			(store_sub, reg22, reg21, 1),
			(str_store_string, s25, "@{s25}^{reg23?Her:His} host contains {reg21} soldier{reg22?s:}."),
		(else_try),
			(str_clear, s25),
		(try_end),
			  ...
			  (try_begin),
				...
				(ge, ":relation", -10),
					...
					(try_for_range, ":stack_no", 0, ":stack_count"),
						...
						(call_script,"script_change_player_relation_with_troop", ":troop_no", 1, 1),
					(try_end),
...
##                                                   (change_screen_map_conversation, ":cur_guard")
      ...
      ("town_sneak",
       ...
       [
		 ...
		 ## WINDYPLAINS+ ## - TROOP ABILITY: BONUS_USEFUL_CONTACTS - Improves chance to sneak into a town by 20% (stacking).
		 (try_for_range, ":troop_no", companions_begin, companions_end),
			(main_party_has_troop, ":troop_no"),
			(call_script, "script_cf_ce_troop_has_ability", ":troop_no", BONUS_USEFUL_CONTACTS),
			(val_add, ":random_chance", 20),
		 (try_end),
		 ## WINDYPLAINS- ##
         (try_begin),
      ...
      ("castle_start_siege",
       [
		   ...
		   ## WINDYPLAINS+ ## - Allow laying siege to a location without being part of a faction.
...
#                                                       (set_jump_mission,"mt_town_default"),
   ...
   (
    ...
    [
      ...
      ("guard_leave",[],
       ## BARNOLO+ ## - Changed to allow dialog to go back to the town menu.
	   # "Leave.",
       # [(change_screen_return,0)]),
       "Go Back.",
       [(jump_to_menu, "mnu_castle_outside")]),
    ...
    [
      ...
      ("castle_lead_attack",
       ...
       [
		   ...
		   ## WINDYPLAINS+ ## - Insufficient infantry causes defenders to sally.
           (else_try),
             (eq, "$g_player_besiege_town", "$current_town"),
			 (le, ":battle_advantage", 2),
             (assign, ":army_armor_rating", 0),
			 (assign, ":army_count", 0),
			 (party_get_num_companion_stacks, ":stack_capacity", "p_main_party"),
			 (try_for_range, ":stack_no", 0, ":stack_capacity"),
				(party_stack_get_troop_id,      ":stack_troop", "p_main_party", ":stack_no"),
				(neg|troop_is_hero, ":stack_troop"),
				(party_stack_get_size,          ":stack_size", "p_main_party", ":stack_no"),
				(party_stack_get_num_wounded,   ":stack_wounded", "p_main_party", ":stack_no"),
				(val_sub, ":stack_size", ":stack_wounded"),
				(call_script, "script_hub_troop_get_armor_rating", ":stack_troop"), # hub_scripts.py
				(assign, ":rating_armor", reg1),
				(val_mul, ":rating_armor", ":stack_size"),
				(val_add, ":army_armor_rating", ":rating_armor"),
				(val_add, ":army_count", ":stack_size"),
			 (try_end),
			 (val_max, ":army_count", 1),
			 (val_div, ":army_armor_rating", ":army_count"),
			 (lt, ":army_armor_rating", 300),
			 (eq, "$g_siege_sallied_out_once", 0),
             (set_jump_mission, "mt_castle_attack_walls_defenders_sally"),
             (assign, "$g_siege_battle_state", 0),
             (assign, ":siege_sally", 1),
    ...
    [
      ("build_ladders_cont",[],
       "Do it.", [
		   ...
		   ## WINDYPLAINS+ ## - IMPROVEMENT [ Reinforced Walls ] - Extend siege duration by 50%.
		   (try_begin),
				(party_slot_ge, "$current_town", slot_center_has_reinforced_walls, cis_built),
				(store_div, ":extended_siege_hours", ":hours_takes", 2),
				(val_add, ":hours_takes", ":extended_siege_hours"),
		   (try_end),
		   ## WINDYPLAINS- ##
 ...
 Your contract has now expired. What will you do?",
    ...
    [
      ...
      ("dont_renew_oath",[],"Become free of your bond.",
       [
         (call_script, "script_player_leave_faction", 1), #1 means give back fiefs
		 ## WINDYPLAINS+ ## - Reset mercenary's next service renew day to 0.  This is used to tell if hte player is a mercenary.
...
##  ),
  ...
  (
    ...
    [
		...
		## WINDYPLAINS+ ## - qst_qp7_freemans_return
		(try_begin),
			(check_quest_active, "qst_qp7_freemans_return"),
			(quest_slot_eq, "qst_qp7_freemans_return", slot_quest_target_center, "$g_encountered_party"),
			(quest_get_slot, ":quest_object_troop", "qst_qp7_freemans_return", slot_quest_object_troop),
			(call_script, "script_get_meeting_scene"), (assign, ":meeting_scene", reg0),
			(modify_visitors_at_site,":meeting_scene"),
			(reset_visitors),
			(set_visitor,0, "trp_player"),
    ...
    [
      ...
      [
		...
		## WINDYPLAINS+ ## - Village "manage this village" native menu option is disabled.
		(eq, 1, 0),
		## WINDYPLAINS- ##
        ]
       ,"Manage this village.",
      ...
      [
		...
		## WINDYPLAINS+ ## - Block the "recruit volunteers" menu option in villages.  No longer needed.
		(eq, 1, 0),
		## WINDYPLAINS- ##
       ]
       ,"Recruit Volunteers.",
	...
	## WINDYPLAINS+ ## - Allow meeting the village elder from village menu.
	("dplmc_village_elder_meeting",[
       ...
       ,"Speak with {s21}, the village elder.",
       [
         (try_begin),
           (call_script, "script_cf_enter_center_location_bandit_check"),
         (else_try),
			## WINDYPLAINS+ ## - IMPROVEMENT [ Fishery ] - Ensures a few fish available on the elder and improves his money available.
			(try_begin),
				(party_slot_ge, "$current_town", slot_center_has_fishery, cis_built),
				(set_price_rate_for_item, "itm_smoked_fish", 40),
			(try_end),
			## WINDYPLAINS- ##
           (party_get_slot, ":village_scene", "$current_town", slot_castle_exterior),
           (modify_visitors_at_site,":village_scene"),
	...
	## WINDYPLAINS- ##
	  ...
	  ## WINDYPLAINS+ ## - CMS - Allow storekeeper to auto-purchase food upon village exit.
      #("village_leave",[],"Leave...",[(change_screen_return,0)]),
      ("village_leave",[],"Leave...",
		[
			(try_begin),
			  (eq, "$cms_enable_auto_buying", 1),
			  (party_slot_eq, "$current_town", slot_village_state, 0), # Block buying if the village is looted or infested.
              (neg|party_slot_ge, "$current_town", slot_village_infested_by_bandits, 1),
			  (assign, "$g_buy_foods_when_leaving", 1), # remove this later.  It should be obsolete.
			  (eq, "$g_buy_foods_when_leaving", 1), # remove this later.  It should be obsolete.
  ...
  (
    ...
    [
      ...
      ("village_loot",[(party_slot_eq, "$current_town", slot_village_state, 0),
                       (neg|party_slot_ge, "$current_town", slot_village_infested_by_bandits, 1),
                       ## WINDYPLAINS+ ## - Allow looting of villages regardless of relation.
                       # (store_faction_of_party, ":center_faction", "$current_town"),
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
 ...
 but they offer you all they can find.",
  ...
  (
	...
	(try_for_range, ":improvement_no", native_improvements_begin, center_improvements_end),
		...
		(else_try),
			(str_store_string, s18, "@{!}{s18}^{s0}"), # (str_store_string, s18, "@{!}{s18}, {s0}"),
		(try_end),
    ...
    [
        ("cancel_que", 
			[
				...
				(this_or_next|party_slot_ge, "$g_encountered_party", slot_center_current_improvement_2, 1),
				(party_slot_ge, "$g_encountered_party", slot_center_current_improvement_3, 1),
			],	"Cancel work on a project.",
			[(jump_to_menu, "mnu_cancel_construction_confirm"),]),
		
		("center_build_manor",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_manor),],
			"{s3} manor.",
			[(assign, "$g_improvement_type", slot_center_has_manor),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_fish_pond",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_fish_pond),],
			"{s3} mill.",
			[(assign, "$g_improvement_type", slot_center_has_fish_pond),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_watch_tower",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_watch_tower),],
			"{s3} watch tower.",
			[(assign, "$g_improvement_type", slot_center_has_watch_tower),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_school",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_school),],
			"{s3} school.",
			[(assign, "$g_improvement_type", slot_center_has_school),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_messenger_post",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_messenger_post),],
			"{s3} messenger post.",
			[(assign, "$g_improvement_type", slot_center_has_messenger_post),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_prisoner_tower",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_prisoner_tower),],
			"{s3} prisoner tower.",
			[(assign, "$g_improvement_type", slot_center_has_prisoner_tower),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_garrison",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_garrison),],
			"{s3} local garrison.",
			[(assign, "$g_improvement_type", slot_center_has_garrison),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_grain_crops",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_crops_of_grain),],
			"Plant a grain crops.",
			[(assign, "$g_improvement_type", slot_center_has_crops_of_grain),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_armoury",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_armoury),],
			"{s3} armoury.",
			[(assign, "$g_improvement_type", slot_center_has_armoury),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_small_marketplace",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_small_marketplace),],
			"{s3} marketplace.",
			[(assign, "$g_improvement_type", slot_center_has_small_marketplace),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_improved_roads",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_improved_roads),],
			"Improve the roads.",
			[(assign, "$g_improvement_type", slot_center_has_improved_roads),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_fire_brigade",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_fire_brigade),],
			"Train a fire brigade.",
			[(assign, "$g_improvement_type", slot_center_has_fire_brigade),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_forge",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_forge),],
			"{s3} forge.",
			[(assign, "$g_improvement_type", slot_center_has_forge),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_merc_chapterhouse",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_merc_chapterhouse),],
			"{s3} mercenary chapterhouse.",
			[(assign, "$g_improvement_type", slot_center_has_merc_chapterhouse),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_escape_tunnels",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_escape_tunnels),],
			"{s3} escape tunnels.",
			[(assign, "$g_improvement_type", slot_center_has_escape_tunnels),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_trade_guilds",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_trade_guilds),],
			"Organize a guild of merchants.",
			[(assign, "$g_improvement_type", slot_center_has_trade_guilds),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_castle_library",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_castle_library),],
			"{s3} castle library.",
			[(assign, "$g_improvement_type", slot_center_has_castle_library),
			(jump_to_menu, "mnu_center_improve"),]),
		
		("center_build_training_ground",[(call_script, "script_cf_improvement_can_be_built_here", "$g_encountered_party", slot_center_has_training_grounds),],
			"{s3} training ground.",
			[(assign, "$g_improvement_type", slot_center_has_training_grounds),
			(jump_to_menu, "mnu_center_improve"),]),
  ...
  (
    "center_improve",0,
    "{s19} As the party member with the highest engineer skill ({reg2}), {reg3?you reckon:{s3} reckons} that {reg21?building:repairing} the {s4} will cost you\
 {reg5} denars and will take {reg6} days.",
    ...
    [(call_script, "script_get_improvement_details", "$g_improvement_type"),
	 ...
	 (try_begin),
		(party_slot_ge, "$g_encountered_party", "$g_improvement_type", cis_damaged_20_percent),
		(assign, reg21, 0), # Repairing
		(party_get_slot, ":status_factor", "$g_encountered_party", "$g_improvement_type"),
	 ...
	 (call_script, "script_improvement_takes_additional_reg1_days", "$g_improvement_type"),
	...
	# Diagnostic
	(try_begin),
		(eq, DEBUG_DIPLOMACY, 1), # Cost & time adjustment for building improvements (AI).  (verbose)
    ...
    [
		## WINDYPLAINS+ ## - Cheat for building improvements instantly for free to be used for debugging.
		("improve_cheat",[(ge, DEBUG_IMPROVEMENTS, 1),], "CHEAT: Build or repair improvement instantly for free.", 
			[
				(party_set_slot, "$g_encountered_party", "$g_improvement_type", cis_built),
				(call_script, "script_improvement_completion_benefits", "$g_improvement_type", "$g_encountered_party"),
				(jump_to_menu,"mnu_center_manage"),
			]),
		## WINDYPLAINS- ##
	  ...
	  ("improve_cont",[(store_troop_gold, ":cur_gold", "trp_player"),
       ...
       "Go on.", [
					...
					(try_for_range, ":improvement_slot", slot_center_current_improvement_1, slot_center_improvement_end_hour_1),
						...
						(assign, ":continue", 0),
						(party_set_slot, "$g_encountered_party", ":improvement_slot", "$g_improvement_type"),
						(store_current_hours, ":cur_hours"),
						...
						(store_add, ":start_time_slot", ":improvement_slot", 7),
						(party_set_slot, "$g_encountered_party", ":start_time_slot", ":cur_hours"),
					(try_end),
					(call_script, "script_improvement_apply_special_cost", "$g_improvement_type"),
					(jump_to_menu,"mnu_center_manage"),
					## WINDYPLAINS- ##
                ]),
...
#      (call_script, "script_loot_player_items", 0),
 ...
 Or perhaps, it was given to them by someone who wanted to arrange a suitable ending to your life.",
    ...
    [
      ("continue",[],"Continue...",[
			## WINDYPLAINS+ ## - Fixing redirect from post bandit fight to either town or village menu.
			# (change_screen_return),
			(try_begin),
  ...
  (
    ...
    [
      ...
      (else_try),
        ...
        (try_begin),
          (gt, ":lord", 0),
          (call_script, "script_change_player_relation_with_troop", ":lord", -3, 1),
      ...
      ("take_supplies",[],"Take the supplies.",
       [
         (try_begin),
           ...
           (else_try),
             (call_script, "script_change_player_relation_with_center", "$current_town", -3),
			 ## WINDYPLAINS+ ## - Ticket #1443 - Taking supplies from a village causes relation damage with their faction.
         ...
         (try_begin),
           (gt,  ":village_lord", 1),
          (call_script, "script_change_player_relation_with_troop", ":village_lord", -2, 1),
          (try_end),
         (party_get_slot, ":merchant_troop", "$current_town", slot_town_elder),
    ...
    [
       ...
       (else_try),
         ...
         (try_begin),
		   ...
		   ## WINDYPLAINS+ ## - Consequences of raiding a village.
		   # (call_script, "script_diplomacy_player_loots_village_consequences", "$current_town"), # diplomacy_scripts.py
		   ## WINDYPLAINS- ##
           (party_get_slot, ":town_lord", "$current_town", slot_town_lord),
           (gt, ":town_lord", 0),
           (call_script, "script_change_player_relation_with_troop", ":town_lord", -4, 1),
         (try_end),
         (jump_to_menu, "mnu_village_loot_no_resist"),
       (else_try),
         (eq, "$g_battle_result", -1),
...
#NPC companion changes end
    ...
    [
      ("village_loot",[], "Plunder the village, then raze it.",
       [
		  ...
		  ## WINDYPLAINS+ ## - Roaming Raids & Sieges - Credit: Caba'drin.
 ...
 and then put the buildings to the torch. From the coins and valuables that are found, you get your share of {reg1} denars.",
    ...
    [
        ...
        (try_begin),
          (gt,  ":village_lord", 0),
          (call_script, "script_change_player_relation_with_troop", ":village_lord", -5, 1),
        (try_end),
        ...
        (call_script, "script_change_player_party_morale", ":morale_increase", PMR_LOOTED_VILLAGE),
      ...
      ("continue",[], "Continue...",
       [
          ...
          (try_begin),
            ...
            (party_set_slot, "$current_town", slot_village_number_of_cattle, ":num_cattle"),
          (try_end),
		  ## WINDYPLAINS+ ## - Replaced native code for determining loot from a raided village due to a native 1.143 bug.
          # (troop_clear_inventory, "trp_temp_troop"),
          ...
          # (try_for_range, ":cur_goods", trade_goods_begin, trade_goods_end),
			...
			# (val_max, ":cur_price", 1), ## WINDYPLAINS+ ## - Prevent Div/0 errors.
          ...
          # (try_for_range, ":cur_goods", trade_goods_begin, trade_goods_end),
			...
			# (val_max, ":total_probability", 1), ## WINDYPLAINS+ ## - Prevent Div/0 errors.
		  ...
		  (troop_clear_inventory, "trp_temp_troop"),
			...
			(try_for_range, ":trade_item", trade_goods_begin, trade_goods_end),
				# Come up with a value for the item.
				(item_get_value, ":item_value", ":trade_item"),
				(val_max, ":item_value", 50), # Catch our 0 value items.
				
				# See how many of this item exist for the local goods merchant.  Add -15% value for each found.  Maximum discount of 75%.
				# If none of these items exist then double the value of the item.
				...
				(store_item_kind_count, ":item_count", ":trade_item", ":troop_merchant"),
				(assign, reg31, ":item_count"),
				(try_begin),
					(le, ":item_count", 0),
					(val_mul, ":item_value", 2),
				(else_try),
					(val_sub, ":item_count", 1),
					(store_mul, ":value_discount", ":item_count", 15),
					(val_clamp, ":value_discount", 0, 76),
					(val_mul, ":value_discount", ":item_value"),
					(val_div, ":value_discount", 100),
					(val_sub, ":item_value", ":value_discount"),
				(try_end),
				(assign, reg32, ":item_value"),
				
				(store_div, ":probability", 1500, ":item_value"),
				(val_clamp, ":probability", 3, 31),
				(assign, reg33, ":probability"),
				
				(set_item_probability_in_merchandise, ":trade_item", ":probability"),		
			(try_end),
			
			# Set the number of looted items.
			(party_get_skill_level, ":looting", "p_main_party", "skl_looting"),
			(val_mul, ":looting", 2),
			(store_add, ":looted_item_count", ":looting", 25),
			(troop_add_merchandise, "trp_temp_troop", itp_type_goods, ":looted_item_count"),
			(troop_sort_inventory, "trp_temp_troop"),
			## WINDYPLAINS+ ## - Autoloot - Redirect to menu vs. looting screen.
			# Original code section by Fisheye and from Custom Commander mod.
			# Autoloot: Instead of just displaying the loot screen, we display a loot management menu instead
			(call_script, "script_als_autoloot_redirect", "mnu_village_loot_complete"),
			## WINDYPLAINS- ##
			# (change_screen_loot, "trp_temp_troop"),
			## WINDYPLAINS- ## AUTOLOOT
        ]),
    ],
  ),
  (
  ...
  (
    ...
    [    
       (try_begin),
		...
		## WINDYPLAINS+ ## - Add town lord's mesh to the menu. (Custom Commander)
		(try_begin),
          (party_get_slot, ":center_lord", "$current_town", slot_town_lord),
          (ge, ":center_lord", 0),
          (set_fixed_point_multiplier, 100),
        ...
        (try_end),
		## WINDYPLAINS- ##
		## WINDYPLAINS+ ## - Reset tournament entries if this is a new location from the last tournament.
		(try_begin),
			(neq, "$tpe_tournament_last_location", "$current_town"),
			(assign, "$tpe_tournament_entries", 0),
		(try_end),
		## WINDYPLAINS- ##
        (call_script, "script_update_center_recon_notes", "$current_town"),
...
##            (str_store_string,s11,"@ The townsfolk here have declared their independence."),
        ...
        (try_begin),
          ...
          (neg|is_currently_night),
		  ## WINDYPLAINS+ ## - Limit tournament entries to 3 during a feast.
		  (store_faction_of_party, ":center_faction", "$current_town"),
		  (faction_slot_eq, ":center_faction", slot_faction_ai_state, sfai_feast),
		  (faction_slot_eq, ":center_faction", slot_faction_ai_object, "$current_town"),
		  (eq, "$tpe_tournament_last_location", "$current_town"),
		  (lt, "$tpe_tournament_entries", 3),
		  ## WINDYPLAINS- ##
    ...
    [
      ...
      [        
        ...
        (try_begin),
		  ...
		  (neq, "$morale_last_feast_center", "$current_town"),
		  (assign, "$morale_last_feast_center", "$current_town"),
		  (call_script, "script_change_player_party_morale", 5, PMR_ATTENDED_FEAST),
        (try_end),
      ...
      ("join_tournament", 
		## WINDYPLAINS+ ## - TOURNAMENTS - Limit entries during a feast.
		[
			(neg|is_currently_night),
			(party_slot_ge, "$current_town", slot_town_has_tournament, 1),
			## Either we have participated in less than three tournaments or the tournament invitation quest is active.
			(assign, ":override_limit", 0),
			(try_begin),
				...
				(assign, ":override_limit", 1),
			(try_end),
			(this_or_next|lt, "$tpe_tournament_entries", 3),
			(eq, ":override_limit", 1),
		]
		## WINDYPLAINS- ##
       ,"Join the tournament.",
       [
		   ...
		   ## WINDYPLAINS+ ## - TMT_GLADIATOR morality & tournament limiter.
		   (call_script, "script_objectionable_action", tmt_gladiator, "str_participate_tournament"),
			(assign, "$tpe_tournament_last_location", "$current_town"),
			(val_add, "$tpe_tournament_entries", 1),
		   ## WINDYPLAINS- ##
		   ## WINDYPLAINS+ ## - QUEST INSERT #1: qst_floris_active_tournament
       ...
       [
         ...
         (else_try),
           ...
           (else_try),
             ...
             (assign,reg0,"trp_n_vaegir_skirmisher"), # "trp_vaegir_infantry"),
             (assign,reg1,"trp_n_vaegir_skirmisher"), # "trp_vaegir_infantry"),
             (assign,reg2,"trp_n_vaegir_bowman"), # "trp_vaegir_archer"),
             (assign,reg3,"trp_n_vaegir_guard"), # "trp_vaegir_footman"),
			 ## WINDYPLAINS- ##
           (try_end),
           (shuffle_range,0,4),
      ...
      ("town_tavern",[
          ...
          (this_or_next|eq,"$entry_to_town_forbidden",0),
          (eq, "$sneaked_into_town",1),
		  ## WINDYPLAINS+ ## - Display reminder to turn in wine.
		  (str_store_string, s41, "@Visit the tavern."),
		  (try_begin),
			(check_quest_active, "qst_deliver_wine"),
			(neg|check_quest_succeeded, "qst_deliver_wine"),
			(quest_slot_eq, "qst_deliver_wine", slot_quest_target_center, "$current_town"),
			(try_begin),
				(quest_slot_eq, "qst_deliver_wine", slot_quest_target_item, "itm_quest_wine"),
				(str_store_string, s41, "@{s41} (Deliver Wine)"),
			(else_try),
				(quest_slot_eq, "qst_deliver_wine", slot_quest_target_item, "itm_quest_ale"),
				(str_store_string, s41, "@{s41} (Deliver Ale)"),
			(try_end),
		  (try_end),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
			 ...
			 ## WINDYPLAINS+ ## - Tavern mercenaries are disabled.
...
#           (scene_slot_eq,  ":scene", slot_scene_visited, 1), #check if scene has been visited before to allow entry from menu. Otherwise scene will only be accessible from the town center.
       ...
       [
           ...
           (else_try),
			...
			## WINDYPLAINS+ ## - Extend choice of arena to arena fights.
			...
			(reset_visitors),
			(try_for_range, ":entry_point", 0, 32),
				(store_random_in_range, ":arena_troop", mercenary_troops_begin, mercenary_troops_end),
				(set_visitor, ":entry_point", ":arena_troop"),
			(try_end),
			(store_sub, ":town_offset", "$current_town", towns_begin),
			(store_add, ":troop_arena_master", "trp_town_1_arena_master", ":town_offset"),
			(set_visitor, 52, ":troop_arena_master"),
			## WINDYPLAINS- ##
             (set_jump_entry, 50),
     ...
     ("town_enterprise",
      ...
      [
		...
		(try_end),
		## WINDYPLAINS+ ## - Give bodyguards a talk context so that they don't try to rejoin the party.
		(assign, "$talk_context", tc_tavern_talk),
		## WINDYPLAINS- ##
...
#			(troop_slot_eq, ":lady_guardian", slot_lord_granted_courtship_permission, -1),
	...
	(gt, "$love_interest_in_town", 0),
      ...
      [
        ...
        (try_begin),
          ...
          (assign, reg0, 0),
        (try_end),
		## WINDYPLAINS+ ## - Village "manage this center" native menu option is disabled.
		(eq, 1, 0),
		## WINDYPLAINS- ##
       ],
	...
	## WINDYPLAINS+ ## - Allow accessing the guild master from the town menu.
...
##          ]),
      ...
      [
		(assign, "$g_permitted_to_center",0),
		(change_screen_return,0),
		## WINDYPLAINS+ ## - Nissa objects to leaving towns with tournaments in progress.
		(try_begin),
			(main_party_has_troop, NPC_Odval),                                                            # Nissa is in the party.
			(neg|troop_slot_eq, NPC_Odval, slot_troop_intro_quest_complete, floris_story_arc_successful), # Once a story arc is completed she's permanently loyal so doesn't complain.
			(party_slot_ge, "$current_town", slot_town_has_tournament, 1),                                # A tournament is currently active.
			(neg|is_currently_night),                                                                     # Prevent complaining when a tournament can't be running.
			(quest_slot_eq, "qst_floris_active_tournament", slot_quest_target_center, "$current_town"),   # We were notified of this tournament.
			(call_script, "script_objectionable_action", tmt_egotistic, "str_leave_tournament"),
		(try_end),
		## WINDYPLAINS- ##
      ],"Leave Area."),
...
# They must learn field discipline and the steadiness to follow orders in combat before they can be thought to use arms.",
 ...
 but they offer you all they can find to show their gratitude.",
  ...
  (
    ...
    "none",
    [
		## WINDYPLAINS+ ## - Silverstag Emblems - Prevent merchants from keeping an emblem stack sold to them.
		(call_script, "script_emblem_remove_from_merchants", "$g_encountered_party"), # emblem_scripts.py
		## WINDYPLAINS- ##
	],
    [
      ("assess_prices",
       ...
       [
		...
		## WINDYPLAINS+ ## - Add access to the trade ledger.
		("view_trade_ledger",
			[
				(store_item_kind_count, reg0, "itm_book_trade_ledger"),
				(ge, reg0, 1),
				(try_begin),
					(neg|troop_slot_ge, "trp_player", slot_troop_trade_ledger, 1),
					(disable_menu_option),
					(str_store_string, s1, "@ (access prices to enable)"),
				(else_try),
					(str_clear, s1),
				(try_end),
			],"View your trade ledger.{s1}",
      ...
      ("trade_with_goods_merchant",[(party_slot_ge, "$current_town", slot_town_merchant, 1)],
       ...
       [
           ## WINDYPLAINS+ ## - Capture original inventory for trading experience.
		   # (call_script, "script_trading_copy_troop_inventory_to", "trp_player", "trp_temp_array_b"),
		   ## WINDYPLAINS- ##
	 ...
	 ("back_to_town_menu",[],"Head back.",
       [
           ## WINDYPLAINS+ ## - Capture any purchases made for trading experience.
		   # (try_for_range, ":trade_good", trade_goods_begin, trade_goods_end),
			# (call_script, "script_trading_compare_exchange_of_item", "trp_temp_array_b", "trp_player", ":trade_good"),
		   # (try_end),
		   # # Now overwrite our original inventory with the new one.
		   # (call_script, "script_trading_copy_troop_inventory_to", "trp_player", "trp_temp_array_b"),
		   ## WINDYPLAINS- ##
		   (jump_to_menu,"mnu_town"),
        ]),
...
##	(call_script, "script_merchant_road_info_to_s42", "$g_encountered_party"),
    ...
    [
      ("continue",[],"Continue...",
       [
           ## WINDYPLAINS+ ## - Raise intelligence on a location when accessing prices.
		   (call_script, "script_diplomacy_set_party_intel_level", "$current_town", 2, 1),
		   ## WINDYPLAINS- ##
		   (assign,"$auto_enter_town", "$current_town"),
    ...
    [
      ("ransom_accept",[],"Accept the offer.",
       [(troop_add_gold, "trp_player", reg12),
        ...
        (try_begin),
            (troop_get_type, ":is_female", "trp_player"),
            (eq, ":is_female", 1),			 ## 1.151 ##			

            (get_achievement_stat, ":number_of_lords_sold", ACHIEVEMENT_MEN_HANDLER, 0),  ## 1.151 ##
            (val_add, ":number_of_lords_sold", 1),
            (set_achievement_stat, ACHIEVEMENT_MEN_HANDLER, 0, ":number_of_lords_sold"),  ## 1.151 ##			
      ...
      ("ransom_reject",[],"Reject the offer.",
       [
        (call_script, "script_change_player_relation_with_troop", "$g_ransom_offer_troop", -4, 1),
		## WINDYPLAINS+ ## - Rejecting a ransom offer does not incur a honor penalty if at war.
        #(call_script, "script_change_player_honor", -1),
		## WINDYPLAINS- ##
...
#####################################################################
    ...
    [
      ("continue",[],"Continue...",
       [
           ...
           (try_end),
           (call_script, "script_set_parties_around_player_ignore_player", 8, 12), #it was radius:2 and hours:4, but players make lots of complains about consequent battle losses after releases from captivity then I changed this.   ## 1.151 ##
           (assign, "$g_player_icon_state", pis_normal),
      ...
      ("continue",[],"Continue...",
       [
           ...
           (try_begin),
             ...
             (party_relocate_near_party, "p_main_party", "$capturer_party", 2),
           (try_end),
           (call_script, "script_set_parties_around_player_ignore_player", 8, 12), #it was radius:2 and hours:12, but players make lots of complains about consequent battle losses after releases from captivity then I changed this.   ## 1.151 ##
           (assign, "$g_player_icon_state", pis_normal),
           (set_camera_follow_party, "p_main_party"),
           (rest_for_hours, 0, 0, 0), #stop resting
           (change_screen_return),
 ...
 he proposes to free you in return for {reg5} denars of your hidden wealth. You decide to...",
    ...
    [
      ...
      [
        ...
        (try_end),
        (call_script, "script_set_parties_around_player_ignore_player", 8, 12), #it was radius:2 and hours:6, but players make lots of complains about consequent battle losses after releases from captivity then I changed this.   ## 1.151 ##
        (assign, "$g_player_icon_state", pis_normal),
        (set_camera_follow_party, "p_main_party"),
        (rest_for_hours, 0, 0, 0), #stop resting
        (change_screen_return),
 ...
 and are instructed to raise more men with all due haste if you do not have enough.^^{reg4?Her:His} message indicates {s9}.",
    "none",
    [
        (set_background_mesh, "mesh_pic_messenger"),
...
#        (call_script, "script_change_player_relation_with_troop", ":faction_marshall", -3, 1),
      ],
    [
      ("continue",[],"Continue...",
       [(change_screen_return),
 ...
 He will surely be offended if you do not take the offer...",
    ...
    [
      ...
      ("faction_reject",[],"Decline the invitation.",
       [(call_script, "script_change_player_relation_with_troop", "$g_invite_faction_lord", -3, 1),
       ...
       [
         (try_begin),
           ...
           (try_for_range, ":cur_troop", active_npcs_begin, active_npcs_end),
             ...
             (try_end),
			 ## WINDYPLAINS+ ## - Custom Vassal Titles - Ladies change titles when closest relative changes factions.
			 (try_for_range, ":kingdom_lady", kingdom_ladies_begin, kingdom_ladies_end),
				...
				(eq, ":closest_male_relative", ":cur_troop"),
				(troop_set_faction, ":kingdom_lady", "$g_notification_menu_var1"),
				# (troop_slot_eq, ":kingdom_lady", slot_troop_prisoner_of_party, -1),
				# (troop_set_slot, ":kingdom_lady", slot_troop_cur_center, ":new_center"),
				(call_script, "script_troop_set_title_according_to_faction", ":kingdom_lady", "$g_notification_menu_var1"),
			 (try_end),
			 ## WINDYPLAINS- ##
    ...
    [
      ...
      [      
        ...
        (try_begin),
		  ...
		  # Original code section by Fisheye and from Custom Commander mod.
          # Autoloot: Instead of just displaying the loot screen, we display a loot management menu instead
          (call_script, "script_als_autoloot_redirect", "mnu_bandit_lair"),
          #end Autoloot
          ## WINDYPLAINS- ##
		  #(change_screen_loot, "trp_temp_troop"),
        (try_end),
  ...
  ("kingdom_strength_report",0,
    ...
    [
		...
		(try_for_range, ":kingdom", kingdoms_begin, kingdoms_end),
			...
			(call_script, "script_diplomacy_rate_kingdom_strength", ":kingdom"),
			(str_store_string, s32, "@{s32}Faction: {s31} = {reg31} rating^+{reg32} from centers ^+{reg33} from vassals ^+{reg34} average vassal relation ^{reg35} active wars.^^"),
		(try_end),
...
("stealth_report",0,
	...
	[
		...
		(store_mul, ":penalty_size", ":party_size", 4),
		(try_begin),
			(gt, ":penalty_size", 0),
			(str_store_string, s2, "@{!} -"),
		(else_try),
			(str_store_string, s2, "@{!} +"),
		(try_end),
		(val_sub, ":stealth_chance", ":penalty_size"),
		
		# TROOP EFFECT: BONUS_STEALTHY
		(call_script, "script_ce_get_party_stealth_rating", "p_main_party"), # combat_scripts.py
		(assign, ":bonus_stealthy", reg1),
		(try_begin),
			(gt, ":bonus_stealthy", 0),
			(str_store_string, s3, "@{!} +"),
		(else_try),
			(str_store_string, s3, "str_space"),
		(try_end),
		(val_add, ":stealth_chance", ":bonus_stealthy"),
		...
		(assign, ":encumbrance_orig", reg1),
		(val_div, ":encumbrance", 25),
		(val_sub, ":stealth_chance", ":encumbrance"),
		(try_begin),
			(gt, ":encumbrance", 0),
			(str_store_string, s4, "@{!} -"),
		(else_try),
		...
		(val_clamp, ":stealth_chance", 0, 101),
		
		(assign, reg2, ":penalty_size"),
		(assign, reg3, ":bonus_stealthy"),
		(assign, reg4, ":encumbrance"),
		(assign, reg10, ":stealth_chance"),
		(assign, reg11, ":encumbrance_orig"),
		(str_store_string, s1, "@Current party stealth chance is {reg10}%.^Current party stealth modifiers are:^^Base chance:  +100%^Party Size: {s2}{reg2}%^Stealthy Ability: {s3}{reg3}%^Encumbrance: {s4}{reg4}% ({reg11} lbs)^Distance: Variable^^TOTAL:  {reg10}%"),
	],
	[
		("continue",[],"Continue...",
			[(jump_to_menu, "mnu_party_reports"),]),
  ...
  ("revised_salt_mine",0,
    "You enter a small village area consisting of many worn down wooden buildings huddled together within a fortified wall.  The main dirt road leads you toward a central building with a sign depicting, '{s21}'s Outpost'.  To the west the mouth of a large cave is visible with carts emerging, pushed out by weary workmen, towards a large warehouse behind the outpost.  Armed men stand by with a vigilant eye towards the crowd and your party as you enter.",
    "none",
    [
		(assign, "$current_town","$g_encountered_party"),
		(try_begin),
			(eq, "$g_encountered_party", "p_salt_mine"),
			(str_store_troop_name, s21, salt_mine_npc),
		(else_try),
			(eq, "$g_encountered_party", "p_iron_mine"),
			(str_store_troop_name, s21, NPC_IRON_MINE_MERCHANT),
		(try_end),
	],
    [
      ("salt_mine_speak",
		[
			(eq, "$g_encountered_party", "p_salt_mine"),
			(str_store_troop_name, s21, salt_mine_npc),
		],"Speak with {s21} the Trader.",
		[
			(change_screen_trade, salt_mine_npc),
		]),
      ...
      ("iron_mine_speak",
		...
		[
			(change_screen_trade, NPC_IRON_MINE_MERCHANT),
		]),
	  
      ("salt_mine_price_check",[(ge, BETA_TESTING_MODE, 2),],"Check prices with other centers for salt.",
		[
			## DEFINE BUYING ITEM.
			(assign, ":item_no", "itm_salt"),
			
			## BUYING PRICE
			(call_script, "script_cms_get_item_value_with_imod", ":item_no", imod_plain, CMS_AUTO_BUYING),
			(assign, ":buy_price", reg0),
			(val_div, ":buy_price", 100),
			(call_script, "script_game_get_item_buy_price_factor", ":item_no"),
			(assign, ":buy_price_factor", reg0),
			(val_mul, ":buy_price", ":buy_price_factor"),
			(val_div, ":buy_price", 100),
			(val_max, ":buy_price",1),
			(assign, reg33, ":buy_price"),
			(str_store_party_name, s31, "$g_encountered_party"),
			(str_store_item_name, s32, ":item_no"),
			(display_message, "@DEBUG: {s31} sells {s32} for {reg33} denars.", gpu_green),
			
			## CHECK AGAINST SELLING PRICE ANYWHERE ELSE
			(try_for_range, ":center_no", towns_begin, towns_end),
				(assign, "$g_encountered_party", ":center_no"),
				(call_script, "script_cms_get_item_value_with_imod", ":item_no", imod_plain, CMS_AUTO_SELLING),
				...
				(val_mul, ":sell_price", ":sell_price_factor"),
				(val_div, ":sell_price", 100),
				(val_max, ":sell_price",1),
				(store_sub, ":profit", ":sell_price", ":buy_price"),
				(store_distance_to_party_from_party, reg34, "$g_encountered_party", "p_main_party"),
				(store_div, reg35, ":profit", reg34),
				### DISPLAY ###
				# (neg|is_between, ":profit", -30, 31),
				(assign, reg31, ":profit"),
				(assign, reg32, ":sell_price"),
				(assign, reg33, ":buy_price"),
				(str_store_party_name, s31, ":center_no"),
				(str_store_item_name, s32, ":item_no"),
				(str_clear, s33),
				...
				(try_end),
				(display_message, "@DEBUG: {s31} buys {s32} for {reg32} at {s33}{reg31} profit.", gpu_debug),
				(display_message, "@DEBUG: {s31} is {reg34} distance.  {reg35} profit/distance", gpu_debug),
			(try_end),
			(assign, "$g_encountered_party", "p_salt_mine"),
			(jump_to_menu, "mnu_revised_salt_mine"),
		]),
	  
      ("salt_mine_offload_player",
		[
			(eq, "$cms_role_jailer", "trp_player"),
			(eq, "$g_encountered_party", "p_salt_mine"),
			(party_get_num_prisoner_stacks, ":stacks", "p_main_party"),
			(gt, ":stacks", 0),
		],"Offload prisoners.",
		[
			# Sell prisoners.
			(call_script, "script_cms_offload_prisoners"),
		]),
	  
      ("salt_mine_offload_jailer",
		[
		...
		],"Direct {s21} to oversee the offloading of prisoners.",
		[
			# Sell prisoners.
			(call_script, "script_cms_offload_prisoners"),
		]),
	  
	  ("salt_mine_wait", [], "Wait here for some time.",
		[
			(rest_for_hours_interactive, 24 * 7, 5, 1), #rest while attackable
			(change_screen_return),
		]),
		  
      ("salt_mine_exit",[],"Leave.",[(leave_encounter),(change_screen_return)]),
    ]
  ),
 ]
