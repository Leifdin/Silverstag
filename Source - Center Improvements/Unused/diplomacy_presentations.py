...
# This preference panel will set what kinds of equipment the player wishes allowed in each city's specific tournaments.
  ("auto_loot_checklist", 0, mesh_load_window, [
    ...
    [
		...
		# Text Labels
		(call_script, "script_gpu_create_text_label", "str_alc_main_title", 500, 680, alc_obj_label_main_title, gpu_center_with_outline),
		(overlay_set_color, reg1, gpu_white),
    ...
    [
		...
		############### CONTAINER BEGIN ###############
			...
			(try_for_range, ":troop_no", companions_begin, companions_end),
				...
				# Loop setup
				(try_for_range, ":morality_type", 1, 6),
  ...
  ("cms_party_roles", 0, mesh_load_window, [
    ...
    [
		...
		############### CONTAINER BEGIN ###############
			...
			(call_script, "script_gpu_create_text_label", "str_role_label_storekeeper", ":pos_x_title", ":pos_y_temp", role_obj_label_storekeeper, gpu_left_with_outline),
			(overlay_set_color, reg1, gpu_white),
			...
			(store_add, ":pos_x_temp", ":pos_x_title", 5),
			...
			(call_script, "script_gpu_create_text_label", "str_role_label_storekeeper_reports", ":pos_x_report", ":pos_y_temp", role_obj_label_storekeeper_report, gpu_left_with_outline),
			(overlay_set_color, reg1, gpu_white),
			# Days of food left.
			(assign, ":food_stores", 0),
			(troop_get_inventory_capacity, ":inv_size", "$cms_role_storekeeper"),
			(try_for_range, ":i_slot", 0, ":inv_size"),
				(troop_get_inventory_slot, ":item_no", "$cms_role_storekeeper", ":i_slot"),
				...
				(troop_inventory_slot_get_item_amount, ":food_amount", "$cms_role_storekeeper", ":i_slot"),
				(val_add, ":food_stores", ":food_amount"),
			(try_end),
			(party_get_num_companion_stacks, ":num_stacks","p_main_party"),
			(assign, ":consumption", 0),
			(try_for_range, ":i_stack", 0, ":num_stacks"),
				(party_stack_get_size, ":stack_size","p_main_party",":i_stack"),
				(val_add, ":consumption", ":stack_size"),
			(try_end),
			(val_div, ":consumption", 3),
			(val_max, ":consumption", 1),
			(store_div, reg21, ":food_stores", ":consumption"),
			(val_sub, ":pos_y_temp", 22),
			(store_add, ":pos_x_temp", ":pos_x_report", 5),
			...
			(try_for_range, ":i_slot", 9, ":capacity"),
				...
				(call_script, "script_cms_get_item_value_with_imod", ":item_no", ":item_modifier"),
			...
			## COLUMN 2 ## - Role Title: Companion Name, Description of role.
			(store_add, ":pos_y_temp", ":pos_y", ":pos_y2_title"),
			(call_script, "script_gpu_create_text_label", "str_role_label_jailer", ":pos_x_title", ":pos_y_temp", role_obj_label_jailer, gpu_left_with_outline),
			(overlay_set_color, reg1, gpu_white),
