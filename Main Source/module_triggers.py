...
# Refresh Merchants
  ...
  [    
	...
	## WINDYPLAINS+ ## - BONUS_THRIFTY - Notifies the player when merchant inventories are reset.
	(try_begin),
		(call_script, "script_cf_ce_troop_has_ability", "trp_player", BONUS_THRIFTY),
		(display_message, "@Merchant inventories have been reset throughout the world.", gpu_green),
	(try_end),
	## WINDYPLAINS- ##
  ]),
...
#Check for political issues
			(try_begin), #does npc's opponent pipe up?
				...
				## WINDYPLAINS+ ## - Prevent invalid troop script errors if no opponent exists. (Nissa)
...
#Check for new personality clashes
			...
			(else_try), 
				...
				(try_begin),
					...
					## WINDYPLAINS+ ## - Enhanced Diplomacy - Prevent companions from trying to rejoin the party when they are advisors.
					(troop_slot_eq, ":npc", slot_troop_advisor_role, 0), # Companion is not serving as an advisor anywhere.
					## WINDYPLAINS- ##
...
## WINDYPLAINS+ ## - Reduce CPU load on "Lady of the Lake" achievement.
(6, 0, 0,
   [
     ...
     (try_for_range, ":companion", companions_begin, companions_end),
	   ...
	   (try_for_range, ":i_slot", ek_item_0, ek_head),
		 ...
		 (ge, ":item_id", 0),
	 	 ...
	 	 (this_or_next|eq, ":item_id", "itm_claymore"),
