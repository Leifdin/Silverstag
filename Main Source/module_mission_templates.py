...
common_battle_tab_press = (
  ...
  [
	...
	(else_try),
	  (neg|party_slot_eq, "p_main_party", slot_party_pref_bc_continue, 0), #PBOD Battle Continuation On
...
common_battle_check_victory_condition = (
  ...
  [
	...
	## WINDYPLAINS+ ## - PBOD - Battle Continuation
	(this_or_next|neg|party_slot_eq, "p_main_party", slot_party_pref_bc_continue, 0),
	## WINDYPLAINS- ##
    (neg|main_hero_fallen, 0),
    (set_mission_result,1),
    (display_message,"str_msg_battle_won"),
  ...
  (0, 0, 0,
   [
    ...
    [
	  ...
	  ## WINDYPLAINS+ ## - Reshuffle teams in the practice arena so you're not ganged up on.
		# TRIGGER 24: Practice Arena Team Shuffling
		(3, 0, 0, 
			[	
				(eq, "$g_mt_mode", abm_training),
			],
			[
				# (display_message, "@DEBUG: Team reshuffling has occurred.", gpu_debug),
				(get_player_agent_no, ":player_agent"),
				(try_for_agents, ":agent_no"),
					(agent_is_alive, ":agent_no"),
					(neq, ":agent_no", ":player_agent"), # Player always stays team 6.
					(store_random_in_range, ":new_team", 0, 6),
					(agent_set_team, ":agent_no", ":new_team"),
				(try_end),
			]),
		## WINDYPLAINS- ##
  ]
  ...
  (
    ...
    [(0,mtef_scene_source|mtef_team_0,af_override_horse,0,1,pilgrim_disguise),
     ...
     [
      ...
      (1, 0, ti_once, ## WINDYPLAINS+ ## - Tavernkeeper only warns you once not to shoot in the tavern.
      [
	    (neg|conversation_screen_is_active),
		(eq, "$talk_context", tc_tavern_talk),
		(gt, "$g_main_attacker_agent", 0),
...
#-----------------------------------------------------------------
  ...
  (
    ...
    [
	  ...
	  ## WINDYPLAINS+ ## - Have folks within the castle remove their helmets.
      ...
      [
			...
			(try_for_agents, ":agent_no"),
				...
				(ge, ":item_no", 1), # A valid item number was returned.
				(agent_unequip_item, ":agent_no", ":item_no"),
				### DIAGNOSTIC ###
				# (str_store_troop_name, s31, ":troop_no"),
				# (str_store_item_name, s32, ":item_no"),
				# (troop_get_type, reg31, ":troop_no"),
				# (display_message, "@DEBUG: {s31} has removed {reg31?her:his} {s32} while in the castle.", gpu_debug),
			(try_end),
      ]),
	  ## WINDYPLAINS- ##
      (ti_inventory_key_pressed, 0, 0, 
    ...
    [    
      ...
      [
        ...
        (try_begin),
		  ...
		  ## WINDYPLAINS+ ## - Native Bugfix - Prison breaks causing lords to be on the wrong side of the fight.
          # (agent_set_team, ":agent_no", 0),
          (get_player_agent_no, ":agent_player"),
          (agent_get_team, ":team_player", ":agent_player"),
          (agent_set_team, ":agent_no", ":team_player"),
		  ## WINDYPLAINS- ##
          (agent_ai_set_aggressiveness, ":agent_no", 5),
          (troop_set_slot, ":troop_no", slot_troop_will_join_prison_break, 0),
...
#               (val_add, ":end_cond", 1), #removed already, try again
  ...
  (
    ...
    [
	 ...
	 ## WINDYPLAINS+ ## - Companion bodyguards join in for disguise fights.
     (2,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,pilgrim_disguise),
...
#40-49 not used yet
	  ...
	  ## WINDYPLAINS+ ## - Town Arena - Change slot 52 to a visitor source for different arenas.
...
#used for torunament master scene
      ...
      (56, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, [itm_practice_sword, itm_practice_shield, itm_padded_cloth, itm_helmet_with_cap]),
      (57, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, [itm_practice_sword, itm_practice_shield, itm_padded_cloth, itm_helmet_with_cap]),
    ],
    tournament_triggers
  ),
    ...
    [
      ...
      (0, 0, 0, [(try_begin),
                 ...
                 (else_try),
                   ...
                   (set_visitor,5,"trp_n_vaegir_bowman"), # "trp_vaegir_archer"), 
                   (set_visitor,6,"trp_n_vaegir_bowman"), # "trp_vaegir_archer"), 
                   (set_visitor,7,"trp_n_vaegir_bowman"), # "trp_vaegir_archer"), 
				   ## WINDYPLAINS- ##
  ...
  (
    ...
    [
      ...
      (2, 0, ti_once, 
       ...
       ## WINDYPLAINS+ ## - PBOD - Battle Continuation (Bandit Lairs)
	   # (1, 4, ti_once,
       # [
         # (assign, ":continue", 0),
	  ...
	  (1, 4, 0, [],
          [
				...
				(try_begin),
					(main_hero_fallen),
					(call_script, "script_cf_battle_continuation_checks"), # pbod_scripts.py
					(this_or_next|eq, "$relative_of_merchant_is_found", 1),
