...
## WINDYPLAINS+ ## - Original process_operations blocks.
...
def add_variable(variable_string,variables_list,variable_uses):
  found = 0
  # Dunde BEGIN
  if variable_string.startswith('reserved_'):
    print "ERROR: $"+ variable_string + "  Do not use prefix reserved_ as variable name!"
  # Dunde END  
  for i_t in xrange(len(variables_list)):
    if variable_string == variables_list[i_t]:
      found = 1
  ...
  if not found:
    for i_t in xrange(len(variables_list)):
      if variables_list[i_t].startswith('reserved_'):
        found = 1
        print "INFO: " + variables_list[i_t] + " is used as $" + variable_string
        variable_uses[i_t] = variable_uses[i_t] - 2 # changed from 1 to allow the warning message if the global never be used then
        variables_list[i_t] = variable_string
        break  
  # Dunde END
  if not found:
    variables_list.append(variable_string)
