...
#Auto-menu
  (0,
   [          
     ## WINDYPLAINS+ ## - Detect fresh game load.
	 (try_begin),
		(call_script, "script_cf_detect_loaded_game"),
	 (try_end),
	 ## WINDYPLAINS- ##
	 (try_begin),
...
## WINDYPLAINS+ ## - Roaming Raids & Sieges - Credit: Caba'drin.
 (0.25,
	[
		...
		(else_try),
			(ge, ":distance", 3), # raid_distance),
			(map_free),
			(str_store_party_name_link, s1, "$g_player_besiege_town"),
			(display_message, "@You cannot maintain your siege of {s1} from this distance. You risk your lines breaking.", gpu_red),
		(else_try),
			(store_current_hours, ":cur_hours"),
			(ge, ":cur_hours", "$g_siege_method_finish_hours"),
			(neg|is_currently_night),
...
## WINDYPLAINS- ##
  ...
  (24 * 7,
   [
	 ...
	 ## WINDYPLAINS+ ## - Track when the next payday is.
	 (store_current_hours, ":hours"),
	 (val_add, ":hours", 7*24),
	 (assign, "$budget_next_payday", ":hours"),
	 ## WINDYPLAINS- ##
     (try_begin),
...
#    (start_presentation, "prsnt_custom_banner"),
  ...
  (24,
   [
      ## WINDYPLAINS+ ## - Party Morale - Daily adjustment (Native) - Blocked when alternate system used.
	  (neq, "$diplomacy_use_alt_morale", 1), # Not using the Enhanced Diplomacy morale system.
	  ## WINDYPLAINS- ##
...
#Party AI: pruning some of the prisoners in each center (once a week)
  ...
  (24,
   [
     ...
     (try_for_range, ":center_no", centers_begin, centers_end),
       ...
       (le, ":random", 10),
	   ## 1.153- ##
	   ## WINDYPLAINS+ ## - Enhanced Diplomacy - Prosperity recovery rate (calculation)
	   # Check to see if "WAR TAXATION" decree is reducing recovery or "PERIOD OF RECONSTRUCTION" decree is improving recovery rate.
	   # Check to see if the improvement "IMPROVED ROADS" is built.
		(store_faction_of_party, ":faction_no", ":center_no"),
		(assign, ":recovery_rate", 100),
		(try_begin),
			(party_slot_eq, ":center_no", slot_center_has_improved_roads, cis_built),
			(val_add, ":recovery_rate", 100),
		(try_end),
		(try_begin),
			(faction_slot_eq, ":faction_no", slot_faction_decree_reconstruction, 1), # "PERIOD OF RECONSTRUCTION" decree active.
			(store_sub, ":reconstruction_bonus", diplomacy_reconstruction_recovery_bonus, 1),
			(val_mul, ":reconstruction_bonus", 100),
			(val_add, ":recovery_rate", ":reconstruction_bonus"), # Should be +200%
		(try_end),
		(try_begin),
			(faction_slot_eq, ":faction_no", slot_faction_decree_war_taxes, 1), # "WAR TAXATION" decree active.
			(val_div, ":recovery_rate", diplomacy_war_taxation_recovery_penalty),
		(try_end),
		(val_clamp, ":recovery_rate", 0, 401), # Hard cap at 400%.
		## WINDYPLAINS- ##
       (call_script, "script_get_center_ideal_prosperity", ":center_no"),
       (assign, ":ideal_prosperity", reg0),
       (party_get_slot, ":prosperity", ":center_no", slot_town_prosperity),       ## 1.151 ##
       (try_begin),
	     ## 1.153+ ##
       ...
       (else_try),
         (lt, ":prosperity", ":ideal_prosperity"),		 ## 1.151 ##
		 ## WINDYPLAINS+ ## - Enhanced Diplomacy - Improved prosperity recovery rate. (implementation)
			(try_begin),
				(ge, ":recovery_rate", 100),
				(store_mul, ":recovery_change", 1, ":recovery_rate"),
				(val_div, ":recovery_change", 100),
			(else_try),
				(store_random_in_range, ":roll", 0, 100),
				(ge, ":roll", ":recovery_rate"),
				(assign, ":recovery_change", 0),
			(try_end),
			(val_clamp, ":recovery_change", 0, 5), # Hard cap reinforced at +4 change.
			(ge, ":recovery_change", 1),
			(call_script, "script_change_center_prosperity", ":center_no", ":recovery_change"),
			(val_add, "$newglob_total_prosperity_from_convergence", ":recovery_change"),
			# Native lines.
			# (call_script, "script_change_center_prosperity", ":center_no", 1),
			# (val_add, "$newglob_total_prosperity_from_convergence", 1),
		 ## WINDYPLAINS- ##
  ...
  (6,
   [(store_current_day, ":cur_day"),
	...
	## WINDYPLAINS+ ## - Raise intelligence on a location when resting at a location.
	(try_begin),
		(eq, "$g_last_rest_center", "$current_town"),
		(eq, "$g_town_visit_after_rest", 1),
		(store_random_in_range, ":chance", 0, 3),
		(eq, ":chance", 0),
		(call_script, "script_diplomacy_set_party_intel_level", "$current_town", 1, 1),
	(try_end),
    ## WINDYPLAINS- ##
    (try_begin),
      (neg|map_free),
  ...
  (24,
   ...
   (try_begin),
		...
		(try_begin),
			...
			## WINDYPLAINS+ ## - Prevent popup notifications on every incident declaration if you don't want them.
			# (call_script, "script_add_notification_menu", "mnu_notification_border_incident", ":acting_village", -1),
			(try_begin),
				(this_or_next|eq, "$diplomacy_filter_enabled", 0),
				(this_or_next|eq, "$players_kingdom", ":acting_faction"),
				(eq, "$players_kingdom", ":target_faction"),
				(call_script, "script_add_notification_menu", "mnu_notification_border_incident", ":acting_village", -1),
			(try_end),
			## WINDYPLAINS- ##
		...
		(try_for_range, ":faction_2", kingdoms_begin, kingdoms_end),
			...
			(try_begin),
				...
				(try_begin),
					...
					## WINDYPLAINS+ ## - Prevent popup notifications on every truce declaration if you don't want them.
    ...
    (0.5,	
     [
        ...
        (try_begin),
          ...
          (try_begin),
            (this_or_next|eq, ":num_centers", 0), #Thanks Caba`drin & Osviux ## 1.152 ##
        ...
        ## 1.152- ##
			...
			(try_begin),
              ...
              (str_store_string, s4, "str_lord_defects_ordinary"),
				## WINDYPLAINS+ ## - Diplomacy message filter. (lord defects)
				(try_begin),
				   (call_script, "script_cf_diplomacy_message_filter", ":new_faction", context_faction, ":faction", context_faction),
				   (display_log_message, "@{!}{s4}", reg1),
				(try_end),
				## WINDYPLAINS- ##
              (try_begin),
                (eq, "$cheat_mode", 1),
   ...
   [
      ...
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        ...
        (try_begin),
		  ...
		  ## WINDYPLAINS+ ## - [ Reinforced Walls ] - Extends minimum siege period by 50%.
		  (try_begin),
			(party_slot_ge, ":center_no", slot_center_has_reinforced_walls, cis_built),
		    (assign, ":minimum_wait", 36),
		  (else_try),
		    (assign, ":minimum_wait", 24),
		  (try_end),
		  (try_begin),
            (store_random_in_range, ":rand", 0, 100),
            (lt, ":rand", ":random_up_limit"),
			(gt, ":siege_begin_hours", ":minimum_wait"),#initial preparation
...
#			(display_message, "@{s65}"),
  ...
  # Refresh merchant inventories
   ...
   [
	  ...
	  ## WINDYPLAINS+ ## - Refresh inventory of unique traders.
	  # Trader - Salt Mine
	  (call_script, "script_set_stock_for_item_to_x", salt_mine_npc, "itm_salt", 6),
	  (call_script, "script_set_stock_for_item_to_x", salt_mine_npc, "itm_tools", 2),
	  (call_script, "script_set_stock_for_item_to_x", salt_mine_npc, "itm_leatherwork", 1),
	  # Trade - Iron Mine
	  (call_script, "script_set_stock_for_item_to_x", NPC_IRON_MINE_MERCHANT, "itm_iron", 6),
	  (call_script, "script_set_stock_for_item_to_x", NPC_IRON_MINE_MERCHANT, "itm_tools", 2),
	  (call_script, "script_set_stock_for_item_to_x", NPC_IRON_MINE_MERCHANT, "itm_leatherwork", 1),
	  ## WINDYPLAINS- ##
...
##    ]),
  ...
  (48,
   [
		## WINDYPLAINS+ ## - (CMS) - Modified prisoner escape chances.
		# The escape check script expects ":chance" to be a 0-1000 value.  Natively these values seem abnormally low.
		#(call_script, "script_randomly_make_prisoner_heroes_escape_from_party", "p_main_party", 50), # I'm not sure why this was included natively.  It gives a 5% chance to escape always.
		# Attempt to escape player party modified by gaoler's prisoner management skill.
		(store_skill_level, ":retention_bonus", "skl_prisoner_management", "$cms_role_jailer"),
		(val_mul, ":retention_bonus", 13),
		(store_sub, ":chance", 150, ":retention_bonus"),
...
#    ]),
  ...
  (96, ## WINDYPLAINS+ ## - Increased respawn for party-less lords from 48 to 96 hours.  Natively 48.
   [
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         (troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
   ...
   [
    ...
    (try_for_range, ":unused", 0, ":consumption_amount"),
	  ...
	  ## WINDYPLAINS+ ##
		...
		(store_distance_to_party_from_party, ":distance", "$g_encountered_party", "p_main_party"),
		(lt, ":distance", 3),
		(party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
		# This block intentionally does nothing so that no food is consumed.
		# (try_begin),
			# (str_store_party_name, s31, "$g_encountered_party"),
			# (assign, reg31, ":distance"),
			# (display_message, "@DEBUG (food eating): Player food not consumed due being within {reg31} miles of {s31} (owned).", gpu_debug),
		# (try_end),
	  (else_try),
		(is_between, "$g_encountered_party", towns_begin, towns_end),
		(store_distance_to_party_from_party, ":distance", "$g_encountered_party", "p_main_party"),
		(lt, ":distance", 3),
		(store_div, ":consumption_amount", ":num_men", 2),
		# (try_begin),
			# (str_store_party_name, s31, "$g_encountered_party"),
			# (assign, reg31, ":distance"),
			# (display_message, "@DEBUG (food eating): Player food cut in half due being within {reg31} miles of {s31}.", gpu_debug),
		# (try_end),
		(eq, 1, 0), # Intentionally fail this block so that food is still consumed.
	  (else_try),
	  ## WINDYPLAINS- ##
        (gt, ":available_food", 0),
      ...
      (else_try),
        ...
        (call_script, "script_change_player_party_morale", -3, PMR_PARTY_NOT_FED),
 ...
 #Update how good a target player is for bandits
    ...
    (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
      ...
      (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
        ...
        (try_begin),
		  ...
		  ## WINDYPLAINS+ ## - Diplomacy message filter. (prisoner trade between factions)
		   (try_begin),
			   (call_script, "script_cf_diplomacy_message_filter", ":stack_troop", context_troop, ":center_no", context_center),
			   (display_log_message, "@{s1} of {s3} has been released from captivity.", reg1),
		   (try_end),
		   ## WINDYPLAINS- ##
        (try_end),
      (try_end),
  ...
  (72,
   [
	 ...
	 ## WINDYPLAINS+ ## - All centers now receive recruits.
	 # (try_for_range, ":village_no", centers_begin, centers_end),
     # (try_for_range, ":village_no", villages_begin, villages_end),
...
## WINDYPLAINS+ ## - Altered center improvements to allow queing up to 3 at a time. (commented out native trigger)
  # Checking center upgrades
  # (12,
	# [
		# (try_for_range, ":center_no", centers_begin, centers_end),
			# # Small check added to maintain save game compatibility from native design.  (v0.02 -> v0.03)
			# (try_begin),
				# (neg|party_slot_eq, ":center_no", slot_center_current_improvement, 0),
				# (str_store_party_name_link, s21, ":center_no"),
				# (display_message, "@Save game compatibility note: Improvement being built for {s21} moved to new improvement queing system.", gpu_purple),
				# (party_get_slot, ":improvement", ":center_no", slot_center_current_improvement),
				# (party_get_slot, ":end_hour", ":center_no", slot_center_improvement_end_hour),
				# (party_set_slot, ":center_no", slot_center_current_improvement_1, ":improvement"),
				# (party_set_slot, ":center_no", slot_center_improvement_end_hour_1, ":end_hour"),
			...
			# (try_for_range, ":improvement_slot", slot_center_current_improvement_1, slot_center_improvement_end_hour_1),
				# (party_get_slot, ":cur_improvement", ":center_no", ":improvement_slot"),
				...
				# (party_set_slot, ":center_no", ":improvement_slot", 0),
...
#    (ge, "$g_last_rest_center", 0),
  ...
  (36,
   [
       (call_script, "script_spawn_bandits", 0),
    ]),
  ...
  (24,
   [
    ...
    (try_for_parties, ":cur_party"),
	  ...
	  (party_slot_eq, ":cur_party", slot_party_type, spt_kingdom_hero_party), ## 1.151 ##
...
##      (try_end),
  ...
  (1, [(neg|map_free),
       ...
       (eq, ":book_read", 0),
	   ## WINDYPLAINS+ ## - Reading speed is now altered by character intelligence.
	   (store_div, ":reading_speed", ":int", 2),
       ## WINDYPLAINS- ##
	   ...
	   (try_end),
	   (val_add, ":book_reading_progress", ":reading_speed"),
	   ## WINDYPLAINS- ##
       ...
       (str_clear, s2),
	   ## WINDYPLAINS+ ## - Make it so the effects of reading a book are not instant & permanent.
       (try_begin),
       ...
       (else_try),
         ...
         (str_store_string, s2, "@ Your tracking skill has increased by 1."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_training"),
        (str_store_string, s2, "@ Your training skill has increased by 1."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_firstaid"),
         (str_store_string, s2, "@ Your first-aid skill has increased by 1."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_prison_management"),
         (str_store_string, s2, "@ Your prisoner management skill has increased by 1."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_repair_bonus"),
         (str_store_string, s2, "@ You have become more skilled at repairing damaged structures."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_tax_reduction"), 
         (str_store_string, s2, "@ You have become more adept at handling finances."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_cunning_rule"), 
         (str_store_string, s2, "@ You have gained insight into becoming a more cunning leader."),
		 (call_script, "script_change_player_right_to_rule", 3),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_prosperity_1"), 
         (str_store_string, s2, "@ You have gained new insights into land stewardship."),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_prof75_sword"),
		 (store_proficiency_level, ":prof", "trp_player", wpt_one_handed_weapon),
		 (try_begin),
			(lt, ":prof", 75),
			(troop_set_proficiency, "trp_player", wpt_one_handed_weapon, 75), # WSE
			(str_store_string, s2, "@ You have finally figured out where to stick the pointy end."),
		 (else_try),
			(str_store_string, s2, "@ You refreshed yourself on the basics, but gained little insight beyond that."),
		 (try_end),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_prof75_crossbow"), 
		 (store_proficiency_level, ":prof", "trp_player", wpt_crossbow),
		 (try_begin),
			(lt, ":prof", 75),
			(troop_set_proficiency, "trp_player", wpt_crossbow, 75), # WSE
			(str_store_string, s2, "@ You have finally learned out how these infernal contraptions work."),
		 (else_try),
			(str_store_string, s2, "@ You refreshed yourself on the basics, but gained little insight beyond that."),
		 (try_end),
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_escape_chance"), 
         (str_store_string, s2, "@ You have become more at trapping your enemies in battle."),
       (try_end),
       ## WINDYPLAINS- ## 
  ...
  (24, [(try_for_range, ":cur_quest", all_quests_begin, all_quests_end),
          (try_begin),
            ...
            (try_begin),
			  ...
			  ## WINDYPLAINS+ ## - Prevent looted or infested villages from having their quests fail on duration.
			  (try_begin),
			    (quest_get_slot, ":target_center", ":cur_quest", slot_quest_target_center),
				(quest_get_slot, ":giver_center", ":cur_quest", slot_quest_giver_center),
				(assign, ":change", 1),
				(try_begin),
					(is_between, ":target_center", villages_begin, villages_end),
					(this_or_next|party_slot_eq, ":target_center", slot_village_state, svs_looted),
					(this_or_next|party_slot_eq, ":target_center", slot_village_state, svs_being_raided),
					(party_slot_ge, ":target_center", slot_village_infested_by_bandits, 1),
					(assign, ":change", 0),
					(ge, DEBUG_QUEST_CONDITIONS, 1),
					(str_store_quest_name, s31, ":cur_quest"),
					(str_store_party_name, s32, ":target_center"),
					(display_message, "@CHEAT - Quest '{s31}' duration frozen while {s32} is looted or infested.", gpu_debug),
				(try_end),
				(try_begin),
					(is_between, ":giver_center", villages_begin, villages_end),
					(this_or_next|party_slot_eq, ":giver_center", slot_village_state, svs_looted),
					(this_or_next|party_slot_eq, ":giver_center", slot_village_state, svs_being_raided),
					(party_slot_ge, ":giver_center", slot_village_infested_by_bandits, 1),
					(assign, ":change", 0),
					(ge, DEBUG_QUEST_CONDITIONS, 1),
					(str_store_quest_name, s31, ":cur_quest"),
					(str_store_party_name, s32, ":giver_center"),
					(display_message, "@CHEAT - Quest '{s31}' duration frozen while {s32} is looted or infested.", gpu_debug),
				(try_end),
				(val_sub, ":exp_days", ":change"),
			  (try_end),
			  ## WINDYPLAINS- ##
              (try_begin),
                (eq, ":exp_days", 0),
   ...
   [
     ...
     (try_begin),
       ...
       (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
		 ...
		 (try_begin),
			(call_script, "script_cf_ce_troop_has_ability", ":stack_troop", BONUS_LOYAL), # combat_scripts.py - ability constants in combat_constants.py
			(val_add, ":troop_morale", 20),
		 (try_end),
		 ## WINDYPLAINS- ##
		 ## WINDYPLAINS+ ## - Enhanced Diplomacy - Alter troop desertion values.
		 (try_begin),
			(call_script, "script_cf_common_player_is_vassal_or_greater", 1),
			(faction_get_slot, ":desertion_threshold", ":faction_no", slot_faction_desertion_threshold),
			(faction_get_slot, ":desertion_factor", ":faction_no", slot_faction_desertion_factor),
		 (else_try),
			(assign, ":desertion_threshold", 32),
			(assign, ":desertion_factor", 4),
		 (try_end),
		 (store_add, ":morale_cutoff", ":desertion_threshold", ":desertion_factor"),
		 
         (lt, ":troop_morale", ":desertion_threshold"), # 32),
         (store_sub, ":desert_prob", ":morale_cutoff", ":troop_morale"), # Native morale_cutoff = 36
         (val_div, ":desert_prob", ":desertion_factor"), # 4),
		 ## WINDYPLAINS- ##
...
#           (try_end),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  ## WINDYPLAINS+ ##
  ## CREDIT:  MadVader
  ## PURPOSE: Detect a recently loaded game and update scripts as needed.
  (0,
	[
		(call_script, "script_cf_detect_loaded_game"),
	]),
  ...
  ## WINDYPLAINS+ ##
  ## PURPOSE: Put up a pop-up whenever WSE is detected as not running as a warning to the player.
  (2,
   [
		## DETERMINE IF WSE IS RUNNING.
		(try_begin),
			(is_vanilla_warband),
			(display_message, "@WARNING!!! Your copy of the game is not currently running WSE and will crash!", gpu_red),
			(str_store_string, s21, "@The Warband Script Enhancer (WSE) is not currently functioning.  You need to start the game using the 'WSE Loader' application found in the WSE v3.1.5 folder or you'll run into opcode 8 crashes."),
			(dialog_box, s21, "@WARNING!!!"),
		(try_end),
   ]),
  ## WINDYPLAINS- ##
  
  ## WINDYPLAINS+ ## - Weekly trigger to attempt rebooting inactive lords.
  (24*7,
   [
		(try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
			(neq, ":troop_no", "trp_player"),
			...
			# (display_message, "@{s22} - {s11}.", gpu_debug),
		(try_end),
   ]),
   ## WINDYPLAINS- ##
  ...
  (1,
   [
	...
	# (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
		# (troop_get_slot, ":party_no", ":troop_no", slot_troop_leaded_party),
		# (party_is_active, ":party_no"),
		# (party_set_icon, ":party_no", "icon_swadia_king_a"),
		# (str_store_party_name, s21, ":troop_no"),
		# (display_message, "@{s21} icon changed.", gpu_debug),
	# (try_end),
	## WINDYPLAINS- ##
   ]),
  (999,
