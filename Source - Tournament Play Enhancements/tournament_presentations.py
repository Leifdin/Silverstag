...
#####                                                 OPTION DISPLAY                                                  #####
###########################################################################################################################
("tournament_options_panel", 0, mesh_load_window, [  ## PRESENTATION+ ##
    (ti_on_presentation_load,
      [
		...
		(try_begin),
			...
			(str_store_string, s1, "@You have nothing."),
			(create_text_overlay, "$g_presentation_obj_24", s1, tf_center_justify),
			(position_set_x, pos1, 285), # 80),80),
		...
		(str_store_string, s1, "@Your weapon selection is adequate."),
		(create_text_overlay, "$g_presentation_obj_25", s1, tf_center_justify),
		(position_set_x, pos1, 285), # 80),
		...
		############### OPTIONS CONTAINER BEGIN ###############
			...
			(else_try),
				...
				(try_begin),
					(neq, "$tpe_tournament_mode", tpe_mode_elimination), ## PERFORMANCE MODE ##
					# If in "elimination mode" the difficulty slider (and thus team sizes) cannot be adjusted.
    ...
    (ti_on_presentation_event_state_change,
      [
		...
		(else_try), ####### ALWAYS RANDOMIZE CHECKBOX #######
			...
			(troop_set_slot, "$g_wp_tpe_troop", slot_troop_tournament_always_randomize, ":value"),
			(call_script, "script_tpe_update_presentation"),
			# Update difficulty score.
			(call_script, "script_tpe_get_difficulty_value"),
...
# the next round.
  ("tpe_ranking_display", 0, mesh_load_window, [  ## PRESENTATION+ ##
    (ti_on_presentation_load,
    [
		(set_fixed_point_multiplier, 1000),
		...
		(else_try),
			...
			(position_set_y, pos1, ":pos_y"),
			(store_add, ":text_pos_x", ":pos_x", -15), (position_set_x, pos1, ":text_pos_x"),
			(create_text_overlay, reg1, "@{s15}", tf_left_align|tf_with_outline), # s15 is set in script "tpe_designate_continuing_participants"
			(overlay_set_color, reg1, 0xDDDDDD),
			(overlay_set_position, reg1, pos1),
		(try_end),
		...
		###############################################
			...
			(create_text_overlay, reg1, "@TOURNAMENT AWARDS", tf_center_justify),
			#(overlay_set_color, reg1, 0xDDDDDD),
    ...
    [
		...
		(else_try),
			...
			(try_begin),
				...
				(troop_slot_eq, "trp_player", slot_troop_tournament_flag_to_continue, 0), # Player does not qualify to continue on.
				(jump_to_menu, "mnu_tpe_player_eliminated"),
			(else_try),
				## PERFORMANCE MODE ##
				(lt, "$g_tournament_cur_tier", wp_tpe_max_tournament_tiers),
				(jump_to_menu, "mnu_tpe_town_tournament"),
			(else_try),
				## END OF TOURNAMENT (ANY MODE) ##
...
# the next round.
  ("tpe_final_display", 0, mesh_load_window, [
    ...
    [
		...
		(stop_all_sounds, 1), # Should kill all looping sounds.
		...
		## ELIMINATION MODE ## - Divorce from ranking by score and instead go by who got to the final 3 spots.
		# (try_begin),
			# (eq, "$tpe_tournament_mode", tpe_mode_elimination),
			# (assign, ":troop_first", "$tpe_rank_1"),
			# (assign, ":troop_second", "$tpe_rank_2"),
			# (assign, ":troop_third", "$tpe_rank_3"),
		# (try_end),
		...
		(try_begin),
			...
			(assign, reg32, ":troop_second"),
			(assign, reg33, ":troop_third"),
		...
		(else_try),
			# Reset total wins back to 0 if you lose in this town.
			(party_set_slot, "$current_town", slot_center_tournament_wins, 0),
		(try_end),
		...
		################################ FIRST PLACE ################################
		(create_text_overlay, reg1, "@1st", tf_center_justify),
		#(overlay_set_color, reg1, 0xDDDDDD),
		(position_set_x, pos1, ":pos_x_titles"),
		...
		(create_text_overlay, reg1, "@Tournament Awards", tf_left_align),
		...
		(try_begin),
			...
			(store_random_in_range, "$tpe_award_group", REWARD_GROUPS_BEGIN, REWARD_GROUPS_END),
			### DIAGNOSTIC+ ###
			(try_begin),
				(ge, DEBUG_TPE_LOOT, 1),
				(assign, reg31, "$tpe_award_group"),
				(display_message, "@DEBUG: $tpe_award_group = {reg31}", gpu_debug),
			(try_end),
			### DIAGNOSTIC- ###
			(call_script, "script_tpe_describe_award_group", "$tpe_award_group"),
			
			# OBJ (nil) - Award Group
			(create_text_overlay, reg1, "@Award Type: {s1}", tf_left_align),
			# (store_add, ":pos_y_name", ":pos_y_first", 54),
			(position_set_x, pos1, ":pos_x_gains"),
			(position_set_y, pos1, ":pos_y_name"),
			(overlay_set_position, reg1, pos1),
			
			(val_sub, ":pos_y_name", 22),
		...
		## Questutil_simple_triggers.py - Check if noble recruits join your cause.
		(try_begin),
			(eq, ":troop_first", "trp_player"),
			(eq, "$award_nobles_reasoning", 0),
			(call_script, "script_cf_qus_player_is_vassal", 1),
			(assign, "$award_nobles_reasoning", "str_noble_joins_due_to_tournament"),
		(else_try),
			(eq, ":troop_first", "trp_player"),
			(eq, "$award_nobles_reasoning", 0),
		...
		(try_begin),
			(eq, ":troop_first", "trp_player"),
			(store_sub, ":lead", ":points_first", ":points_second"),
			(gt, ":lead", 8),
		...
		(try_begin),
			...
			(assign, "$tpe_player_won_tournament", 1),
			## QUEST UPDATE: "qp1_score_to_settle"
			(neg|check_quest_active, "qst_score_to_settle"),
			(quest_set_slot, "qst_score_to_settle", slot_quest_giver_troop, -1), # Prevent future dialog that will trigger the quest starting.
		(else_try),
			## QUEST UPDATE: "qp1_score_to_settle"
			(neg|check_quest_active, "qst_score_to_settle"),
			(quest_set_slot, "qst_score_to_settle", slot_quest_giver_troop, ":troop_first"), # Allow for future dialog that will trigger the quest beginning.
		(try_end),
		...
		(try_begin),
			(neq, ":troop_second", -1),
			(create_text_overlay, reg1, "@2nd", tf_center_justify),
			#(overlay_set_color, reg1, 0xDDDDDD),
			(position_set_x, pos1, ":pos_x_titles"),
			...
			# OBJ (nil) - Experience Gain
			(create_text_overlay, reg1, "@{reg6} experience", tf_left_align),
			(store_add, ":pos_y_name", ":pos_y_second", 54), # 32),
			(position_set_x, pos1, ":pos_x_gains"),
			(position_set_y, pos1, ":pos_y_name"),
			(overlay_set_position, reg1, pos1),
			...
			# OBJ (nil) - Renown Gain
			(create_text_overlay, reg1, "@{reg7} renown", tf_left_align),
			(store_add, ":pos_y_name", ":pos_y_second", 32), # 10),
			(position_set_x, pos1, ":pos_x_gains"),
			(position_set_y, pos1, ":pos_y_name"),
			(overlay_set_position, reg1, pos1),
		...
		(try_begin),
			(neq, ":points_third", -1),
			(create_text_overlay, reg1, "@3rd", tf_center_justify),
			#(overlay_set_color, reg1, 0xDDDDDD),
			(position_set_x, pos1, ":pos_x_titles"),
			...
			# OBJ (nil) - Experience Gain
			(create_text_overlay, reg1, "@{reg6} experience", tf_left_align),
			(store_add, ":pos_y_name", ":pos_y_third", 54), # 32),
			(position_set_x, pos1, ":pos_x_gains"),
			(position_set_y, pos1, ":pos_y_name"),
			(overlay_set_position, reg1, pos1),
			...
			# OBJ (nil) - Renown Gain
			(create_text_overlay, reg1, "@{reg7} renown", tf_left_align),
			(store_add, ":pos_y_name", ":pos_y_third", 32), # 10),
			(position_set_x, pos1, ":pos_x_gains"),
			(position_set_y, pos1, ":pos_y_name"),
			(overlay_set_position, reg1, pos1),
		...
		# OBJ #1 - EXIT TOURNAMENT button
		(try_begin),
			(eq, "$tpe_player_won_tournament", 1),
			(eq, wp_tpe_mod_opt_award_items_on_win, 1),
			(str_store_string, s21, "@Collect Prize"),
		(else_try),
			(str_store_string, s21, "@Exit Tournament"),
		(try_end),
		(create_game_button_overlay, "$g_presentation_obj_1", "@{s21}"),
    ...
    [
		...
		(try_begin),
			...
			(try_begin),
				(eq, "$tpe_player_won_tournament", 1),
				(eq, wp_tpe_mod_opt_award_items_on_win, 1), 			# If we don't award items then we want to skip this.
				(troop_set_slot, TPE_OPTIONS, tpe_val_window_mode, 3), 	# Leave this screen to the prize selection one.
				(jump_to_menu, "mnu_tpe_jump_to_rankings"),
			(else_try),
				(jump_to_menu, "mnu_town"),
			(try_end),
      ...
      (ti_on_presentation_load,
       [
		...
		(try_begin),
			...
			## WINDYPLAINS+ ## - Add the STAMINA_BAR to the tournament HUD. (initial creation)
...
##############################################################################
# This preference panel will set what kinds of equipment the player wishes allowed in each city's specific tournaments.
  ("tpe_credits", 0, mesh_load_window, [  ## PRESENTATION+ ##
    (ti_on_presentation_load,
    [
		(set_fixed_point_multiplier, 1000),
		...
		# Create header display
		(call_script, "script_gpu_create_text_label", "str_tci_main_title", ":x_col_label_titles", 680, 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 150),
		(call_script, "script_gpu_create_text_label", "str_tci_main_title", ":x_col_label_titles", 680, 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 150),
		# (call_script, "script_gpu_create_text_label", "str_tci_main_title", 15, 680, 0, gpu_left_with_outline),
		# (overlay_set_color, reg1, gpu_white),
		(call_script, "script_gpu_create_text_label", "str_tci_sub_title", ":x_col_label_titles", 655, 0, gpu_left),
		...
		(call_script, "script_gpu_container_heading", 0, 75, 960, 525, tci_obj_container_info),
			...
			# (overlay_set_color, reg1, gpu_white),
			(val_sub, ":pos_y", 40), # Move the info lines down a little further.
...
###############################################################
####               HALL OF RECORD DISPLAY                  ####
###############################################################
# This preference panel will set what kinds of equipment the player wishes allowed in each city's specific tournaments.
  ("tpe_hall_of_fame", 0, mesh_load_window, [
    (ti_on_presentation_load,
    [
		(set_fixed_point_multiplier, 1000),
		(assign, "$gpu_storage", TPE_OBJECTS),
		(assign, "$gpu_data",    TPE_OBJECTS),
		...
		# Button Definitions
		(call_script, "script_gpu_create_game_button", "str_tpe_label_hor_exit", 895, 15, tdp_obj_button_done),
		(str_store_string, s21, "@Switch Display Mode"),
		(call_script, "script_gpu_create_game_button", "str_hub_s21", 120, 15, records_obj_toggle_log_mode),
		
		# TPE_LOG_MODE_LIST_TOURNAMENTS              = 0
		# TPE_LOG_MODE_LIST_CITIES                   = 1
		# TPE_LOG_MODE_LIST_ONLY_SELECTED_CITY       = 2
		
		# Margins
		(assign, ":x_col_label_titles",     30), # Left aligned.
		(assign, ":x_col_date",             30),
		(store_add, ":x_col_location",   ":x_col_date",        120),
		(store_add, ":x_col_level",      ":x_col_location",    130),
		(store_add, ":x_col_rank",       ":x_col_level",        60),
		(store_add, ":x_col_survived",   ":x_col_rank",         75),
		(store_add, ":x_col_difficulty", ":x_col_survived",    110),
		(store_add, ":x_col_score",      ":x_col_difficulty",  100),
		(store_add, ":x_col_earnings",   ":x_col_score",       130),
		(assign, ":y_row_headers",         610),
		(assign, ":y_row_footers",         120),
		(assign, ":y_row_footers_data",     88),
		
		# Create header display
		(try_begin),
			(eq, "$tournament_log_mode", TPE_LOG_MODE_LIST_CITIES),
			(str_store_string, s21, "@Tournament History"),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", 30, 650, 0, gpu_left),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", 30, 650, 0, gpu_left),
		(else_try),
			(eq, "$tournament_log_mode", TPE_LOG_MODE_LIST_ONLY_SELECTED_CITY),
			(str_store_party_name, s21, "$tournament_town"),
			(str_store_string, s21, "@City of {s21}"),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", 30, 680, 0, gpu_left),
			(call_script, "script_gpu_resize_object", 0, 150),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", 30, 680, 0, gpu_left),
			(call_script, "script_gpu_resize_object", 0, 150),
			(call_script, "script_tpe_store_town_faction_to_reg0", "$tournament_town"),
			(str_store_faction_name, s21, reg0),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", ":x_col_label_titles", 650, 0, gpu_left),
			(call_script, "script_gpu_create_text_label", "str_tpe_s21", ":x_col_label_titles", 650, 0, gpu_left), # Doubled for bold effect.
		(try_end),
		# Dividing Lines
		(call_script, "script_gpu_draw_line", 965, 2, 15, 630, gpu_gray), # horizontal line underneath city & faction names.
		(call_script, "script_gpu_draw_line", 770, 2, 15, 145, gpu_gray), # horizontal line underneath city & faction names.
		(call_script, "script_gpu_draw_line", 2, 650, 785, 75, gpu_gray), # vertical line separating selectable cities from current city settings.
		# Minor Text Headings
		(call_script, "script_gpu_create_text_label", "str_tdp_label_center_list", 850, 650, 0, gpu_center),
		(call_script, "script_gpu_create_text_label", "str_tdp_label_center_list", 850, 650, 0, gpu_center), # Doubled for bold effect.
		# Headers
		(call_script, "script_gpu_create_text_label", "str_tpe_label_date",       ":x_col_date",       ":y_row_headers", 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_level",      ":x_col_level",      ":y_row_headers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_location",   ":x_col_location",   ":y_row_headers", 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_rank",       ":x_col_rank",       ":y_row_headers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_survived",   ":x_col_survived",   ":y_row_headers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_difficulty", ":x_col_difficulty", ":y_row_headers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_score",      ":x_col_score",      ":y_row_headers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_earnings",   ":x_col_earnings",   ":y_row_headers", 0, gpu_right),
		(call_script, "script_gpu_resize_object", 0, 90),
		
		(assign, ":pos_y", 600),
		(assign, ":pos_y_step", 25),
		(try_begin),
			(ge, "$tpe_total_log_entries", 20),
			(store_sub, ":excess_entries", "$tpe_total_log_entries", 20),
			(val_mul, ":excess_entries", ":pos_y_step"),
			(val_add, ":pos_y", ":excess_entries"),
		(try_end),
		
		# MAIN TOURNAMENT DATA CONTAINER
		(call_script, "script_gpu_container_heading", 0, 155, 755, 425, records_obj_data_container),
			(assign, ":total_entries", 0),
			(assign, ":average_rank", 0),
			(assign, ":average_survived", 0),
			(assign, ":average_difficulty", 0),
			(assign, ":average_score", 0),
			(assign, ":total_earnings", 0),
			
			(try_for_range, ":entry", 0, "$tpe_total_log_entries"),
				# Qualify if this is a valid city type.
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_LOCATION),
				(this_or_next|eq, "$tournament_log_mode", TPE_LOG_MODE_LIST_CITIES),
				(eq, "$tournament_town", reg1),
				(val_add, ":total_entries", 1),
				# Get & print data.
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_DATE),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_date", ":x_col_date", ":pos_y", 0, gpu_left),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_LOCATION),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_location", ":x_col_location", ":pos_y", 0, gpu_left),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_LEVEL),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_level", ":x_col_level", ":pos_y", 0, gpu_center),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_RANK),
				(val_add, ":average_rank", reg1),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_rank", ":x_col_rank", ":pos_y", 0, gpu_center),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_SURVIVED),
				(val_add, ":average_survived", reg1),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_survived", ":x_col_survived", ":pos_y", 0, gpu_center),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_DIFFICULTY),
				(val_add, ":average_difficulty", reg1),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_diff_long", ":x_col_difficulty", ":pos_y", 0, gpu_center),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_SCORE),
				(val_add, ":average_score", reg1),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_score", ":x_col_score", ":pos_y", 0, gpu_center),
				(call_script, "script_gpu_resize_object", 0, 75),
				
				(call_script, "script_tpe_get_log_data", ":entry", TPE_LOG_EARNINGS),
				(val_add, ":total_earnings", reg1),
				(call_script, "script_gpu_create_text_label", "str_tpe_data_earnings", ":x_col_earnings", ":pos_y", 0, gpu_right),
				(call_script, "script_gpu_resize_object", 0, 75),
		...
		# Footers
		(call_script, "script_gpu_create_text_label", "str_tpe_label_num_tourny",  ":x_col_date",       ":y_row_footers", 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_avg_rank",    ":x_col_rank",       ":y_row_footers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_avg_survive", ":x_col_survived",   ":y_row_footers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_avg_diff",    ":x_col_difficulty", ":y_row_footers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_avg_score",   ":x_col_score",      ":y_row_footers", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 90),
		(call_script, "script_gpu_create_text_label", "str_tpe_label_total_earn",  ":x_col_earnings",   ":y_row_footers", 0, gpu_right),
		(call_script, "script_gpu_resize_object", 0, 90),
		# Footer data
		# Total tournaments
		(assign, reg1, ":total_entries"),
		(val_max, ":total_entries", 1),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_total_tourny", ":x_col_date", ":y_row_footers_data", 0, gpu_left),
		(call_script, "script_gpu_resize_object", 0, 75),
		# Average rank
		(store_div, reg1, ":average_rank", ":total_entries"),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_rank", ":x_col_rank", ":y_row_footers_data", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 75),
		# Average rounds survived
		(store_div, reg1, ":average_survived", ":total_entries"),
		(assign, reg2, wp_tpe_max_tournament_tiers),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_survived", ":x_col_survived", ":y_row_footers_data", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 75),
		# Average difficulty
		(store_div, reg1, ":average_difficulty", ":total_entries"),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_diff_short", ":x_col_difficulty", ":y_row_footers_data", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 75),
		# Average score
		(store_div, reg1, ":average_score", ":total_entries"),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_score", ":x_col_score", ":y_row_footers_data", 0, gpu_center),
		(call_script, "script_gpu_resize_object", 0, 75),
		# Total earnings
		(assign, reg1, ":total_earnings"),
		(call_script, "script_gpu_create_text_label", "str_tpe_data_earnings", ":x_col_earnings", ":y_row_footers_data", 0, gpu_right),
		(call_script, "script_gpu_resize_object", 0, 75),
		...
		(troop_set_slot, tdp_objects, tdp_obj_centers_end,   ":towns_end"),
		# script_gpu_container_heading   - pos_x, pos_y, size_x, size_y, storage_id
		(call_script, "script_gpu_container_heading", 750, 75, 200, 545, records_obj_city_container),
    ...
    [
		...
		(else_try),
			##### SWITCH DISPLAY MODES ##### records_obj_toggle_log_mode
			(troop_slot_eq, TPE_OBJECTS, records_obj_toggle_log_mode, ":object"),
			(try_begin), ## SINGLE CITY MODE
				(eq, "$tournament_log_mode", TPE_LOG_MODE_LIST_ONLY_SELECTED_CITY),
				(assign, "$tournament_town", "$current_town"),
				(assign, "$tournament_log_mode", TPE_LOG_MODE_LIST_CITIES),
			(else_try), ## ALL CITIES MODE
...
###############################################################
####               TOURNAMENT PRIZE WINDOW                 ####
###############################################################
# This is the prize window you're taken to after winning a tournament to select your prize.
  ("tpe_rewards_window", 0, mesh_load_window, [
    (ti_on_presentation_load,
    [
		(set_fixed_point_multiplier, 1000),
		(assign, "$gpu_storage", TPE_OBJECTS),
		...
		(str_store_string, s21, "@Select A Tournament Prize"),
		(call_script, "script_gpu_create_text_label", "str_hub_s21", 500, 665, tpe2_obj_main_title, gpu_center), # 680
		(call_script, "script_gpu_resize_object", tpe2_obj_main_title, 150),
		# Doubled for bold effect.
		(call_script, "script_gpu_create_text_label", "str_hub_s21", 500, 665, tpe2_obj_main_title, gpu_center), # 680
		(call_script, "script_gpu_resize_object", tpe2_obj_main_title, 150),
		...
		(try_begin),
			...
			(overlay_set_val, "$g_presentation_obj_1", "$tpe_award_group"),
		(else_try),
			## OBJ - TEXT - SELECTED ITEM
			(call_script, "script_tpe_describe_award_group", "$tpe_award_group"),
			(str_store_string, s21, s1),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", 350, 587, tpe2_obj_label_reward_group, gpu_center),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", 350, 587, tpe2_obj_label_reward_group, gpu_center),
		(try_end),
		
		
		###############################
		##   PRIZE SELECTION BLOCK   ##
		###############################
		
		## OBJ - CONTAINER - AVAILABLE ITEMS FOR COMMISSIONING
		(assign, ":y_bottom", 90),
		(assign, ":x_left",   70),
		(assign, ":x_width",  560), # 7 items wide
		(assign, ":y_width",  480), # 6 items high
		
		## OBJ - CONTAINER+ - ITEM SELECTION
		# (call_script, "script_gpu_draw_line", ":x_width", ":y_width", ":x_left", ":y_bottom", gpu_white), # - Footer
		(call_script, "script_gpu_container_heading", ":x_left", ":y_bottom", ":x_width", ":y_width", tpe2_obj_container_items),
		
			## types
			
			(troop_get_slot, ":record_limit", TPE_OBJECTS, tpe2_val_total_records),
			
			(store_div, ":height", ":record_limit", 7),
			(store_mod, ":offset", ":record_limit", 7),
			(val_min, ":offset", 1),
			...
			(assign, ":pos_x", 0),
			(assign, ":slot_no", 0),
			(try_for_range, ":record_no", 0, ":record_limit"),
				(call_script, "script_tpe_award_get_record", ":record_no", 1),
				(assign, ":item_no", reg1),
				(assign, ":imod", reg2),
				
				## item slot
				(call_script, "script_gpu_create_mesh", "mesh_inv_slot", ":pos_x", ":pos_y", 800, 800),
				(troop_set_slot, TPE_AWARD_ARRAY_SLOT_NO, ":slot_no", reg1),
				## item
				...
				(overlay_set_position, reg1, pos1),
				(troop_set_slot, TPE_AWARD_ARRAY_OBJECT_NO, ":slot_no", reg1),
				(troop_set_slot, TPE_AWARD_ARRAY_ITEM_NO, ":slot_no", ":item_no"),
				(troop_set_slot, TPE_AWARD_ARRAY_IMOD,   ":slot_no", ":imod"),
				(val_add, ":pos_x", 80),
				(val_add, ":slot_no", 1),
				(try_begin),
					(ge, ":pos_x", 640), # x8 items wide
					(assign, ":pos_x", 0),
					(val_sub, ":pos_y", 80),
				(try_end),
			(try_end),
		...
		(str_store_string, s21, "@Selected Prize:"),
		(call_script, "script_gpu_create_text_label", "str_hub_s21", 805, 560, tpe2_obj_title_commissions, gpu_center),
		...
		(assign, ":y_step", 25),
		(assign, ":x_col_1", 750),
		(store_add, ":pos_y_temp", ":pos_y", -73),
		
		(troop_get_slot, ":selected_item_no", TPE_OBJECTS, tpe2_val_selected_item_no),
		(call_script, "script_gpu_create_mesh", "mesh_inv_slot", ":pos_x", ":pos_y_temp", 800, 800),
		(try_begin),
			(is_between, ":selected_item_no", TPE_FIRST_ITEM, TPE_LAST_ITEM),
			## OBJ - IMAGE - SELECTED ITEM
			...
			(troop_set_slot, TPE_OBJECTS, tpe2_obj_image_selected_item, reg1),
			
			## OBJ - TEXT - SELECTED ITEM
			(str_store_item_name, s21, ":selected_item_no"),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_selected_item, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_selected_item, 75),
			# Doubled for bold effect.
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_selected_item, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_selected_item, 75),
			
			## OBJ - TEXT - SELECTED MODIFIER
			(troop_get_slot, ":selected_imod", TPE_OBJECTS, tpe2_val_selected_imod),
			(try_begin),
				(is_between, ":selected_imod", TPE_IMODS_BEGIN, TPE_IMODS_END),
			...
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_selected_imod, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_selected_imod, 75),
			
			## OBJ - TEXT - SELECTED VALUE
			(troop_get_slot, reg21, TPE_OBJECTS, tpe2_val_selected_price),
			(store_sub, reg22, reg21, 1),
			(str_store_string, s21, "@{reg21} denar{reg22?s:}"),
			(val_sub, ":pos_y", ":y_step"),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_selected_price, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_selected_price, 75),
			
		(try_end),
		
		
		###############################
		## MOST VALUABLE PRIZE BLOCK ##
		###############################
		## OBJ - TEXT - SELECTED ITEM TITLE
		(str_store_string, s21, "@Most Valuable Prize:"),
		(call_script, "script_gpu_create_text_label", "str_hub_s21", 805, 340, tpe2_obj_title_commissions, gpu_center),
		
		(call_script, "script_tpe_get_most_valuable_record"),
		(call_script, "script_tpe_award_get_record", reg1, 2),
		(assign, ":valuable_item_no", reg1),
		(assign, ":valuable_imod", reg2),
		...
		(call_script, "script_gpu_create_mesh", "mesh_inv_slot", ":pos_x", ":pos_y_temp", 800, 800),
		(try_begin),
			(is_between, ":valuable_item_no", TPE_FIRST_ITEM, TPE_LAST_ITEM),
			## OBJ - IMAGE - "VALUABLE ITEM" IMAGE
			(store_add, ":pos_x_item", ":pos_x", 40),
			(store_add, ":pos_y_item", ":pos_y_temp", 40),
			(call_script, "script_gpu_create_item_mesh", ":valuable_item_no", ":pos_x_item", ":pos_y_item", 800),
			(troop_set_slot, TPE_OBJECTS, tpe2_obj_image_valuable_item, reg1),
			
			## OBJ - TEXT - "VALUABLE ITEM" NAME
			(str_store_item_name, s21, ":valuable_item_no"),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_valuable_item, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_valuable_item, 75),
			# Doubled for bold effect.
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_valuable_item, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_valuable_item, 75),
			
			## OBJ - TEXT - "VALUABLE ITEM" MODIFIER
			(try_begin),
				(is_between, ":valuable_imod", TPE_IMODS_BEGIN, TPE_IMODS_END),
				(call_script, "script_cci_describe_imod_to_s1", ":valuable_imod", 1),
				(str_store_string, s21, "@{s1} (x{s2}%)"),
			(else_try),
				(str_clear, s21),
			(try_end),
			(val_sub, ":pos_y", ":y_step"),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_valuable_imod, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_valuable_imod, 75),
			
			## OBJ - TEXT - "VALUABLE ITEM" PRICE
			(call_script, "script_cms_get_item_value_with_imod", ":valuable_item_no", ":valuable_imod", CMS_AUTO_LOOTING),
			(assign, reg21, reg0),
			(val_div, reg21, 100),
			(store_sub, reg22, reg21, 1),
			(str_store_string, s21, "@{reg21} denar{reg22?s:}"),
			(val_sub, ":pos_y", ":y_step"),
			(call_script, "script_gpu_create_text_label", "str_hub_s21", ":x_col_1", ":pos_y", tpe2_obj_label_valuable_price, gpu_left),
			(call_script, "script_gpu_resize_object", tpe2_obj_label_valuable_price, 75),
		...
		## BUTTONS ##
		#############
		## OBJ - BUTTON - TAKE SELECTED PRIZE
        (str_store_string, s21, "@Take Selected Prize"),
		(call_script, "script_gpu_create_game_button", "str_hub_s21", 805, 400, tpe2_obj_button_take_selected), # TAKE SELECTED PRIZE
		
		## OBJ - BUTTON - TAKE MOST VALUABLE PRIZE
        (str_store_string, s21, "@Take Most Valuable Prize"),
		(call_script, "script_gpu_create_game_button", "str_hub_s21", 805, 180, tpe2_obj_button_take_valuable), # TAKE MOST VALUABLE PRIZE
		
		## OBJ - BUTTON - EXIT
        (str_store_string, s21, "@Exit"),
		(call_script, "script_gpu_create_game_button", "str_hub_s21", 805, 35, tpe2_obj_button_exit), # EXIT
	...
	(ti_on_presentation_mouse_press,
		[
			...
			(try_begin),
				...
				(assign, ":continue", 0),
				(troop_get_slot, ":record_limit", TPE_OBJECTS, tpe2_val_total_records),
				(try_for_range, ":slot_no", 0, ":record_limit"),
					(this_or_next|troop_slot_eq, TPE_AWARD_ARRAY_SLOT_NO, ":slot_no", ":object"),
					(troop_slot_eq, TPE_AWARD_ARRAY_OBJECT_NO, ":slot_no", ":object"),
					(troop_get_slot, ":selected_item", TPE_AWARD_ARRAY_ITEM_NO, ":slot_no"),
					(troop_get_slot, ":selected_imod", TPE_AWARD_ARRAY_IMOD, ":slot_no"),
					(is_between, ":selected_item", TPE_FIRST_ITEM, TPE_LAST_ITEM),
					(troop_set_slot, TPE_OBJECTS, tpe2_val_selected_item_no, ":selected_item"),
					(troop_set_slot, TPE_OBJECTS, tpe2_val_selected_imod, ":selected_imod"),
					(call_script, "script_cms_get_item_value_with_imod", ":selected_item", ":selected_imod", CMS_AUTO_LOOTING),
					(assign, ":price", reg0),
					(val_div, ":price", 100),
					(troop_set_slot, TPE_OBJECTS, tpe2_val_selected_price, ":price"),
					(assign, ":continue", 1),
					(break_loop),
				(try_end),
				(eq, ":continue", 1),
    ...
    (ti_on_presentation_mouse_enter_leave,
		[
			...
			(try_begin),
				...
				(try_for_range, ":slot_no", 0, ":record_limit"),
					(troop_slot_eq, TPE_AWARD_ARRAY_SLOT_NO, ":slot_no", ":object"),
					(troop_get_slot, ":item_no", TPE_AWARD_ARRAY_ITEM_NO, ":slot_no"),
					(troop_get_slot, ":target_obj", TPE_AWARD_ARRAY_OBJECT_NO, ":slot_no"),
					(troop_get_slot, ":imod", TPE_AWARD_ARRAY_IMOD, ":slot_no"),
					(overlay_get_position, pos0, ":target_obj"),
					(show_item_details_with_modifier, ":item_no", ":imod", pos0, 0),
					(assign, "$g_current_opened_item_details", ":slot_no"),
				(try_end),
				
				## SELECTED ITEM - OPEN DETAILS
				(try_begin),
					(troop_slot_eq, TPE_OBJECTS, tpe2_obj_image_selected_item, ":object"),
					(troop_get_slot, ":item_no", TPE_OBJECTS, tpe2_val_selected_item_no),
					(is_between, ":item_no", TPE_FIRST_ITEM, TPE_LAST_ITEM),
					(troop_get_slot, ":imod", TPE_OBJECTS, tpe2_val_selected_imod),
				...
				## VALUABLE ITEM - OPEN DETAILS
				(try_begin),
					(troop_slot_eq, TPE_OBJECTS, tpe2_obj_image_valuable_item, ":object"),
					(call_script, "script_tpe_get_most_valuable_record"),
					(call_script, "script_tpe_award_get_record", reg1, 3),
					(assign, ":valuable_item_no", reg1),
					(assign, ":valuable_imod", reg2),
					(overlay_get_position, pos0, ":object"),
					(show_item_details_with_modifier, ":valuable_item_no", ":valuable_imod", pos0, 100),
					(assign, "$g_current_opened_item_details", ":object"),
				(try_end),
    ...
    [
		...
		(try_begin),
			##### BUTTON - EXIT #####
			(troop_slot_eq, TPE_OBJECTS, tpe2_obj_button_exit, ":object"),
			(presentation_set_duration, 0),
			(jump_to_menu, "mnu_town"),
			
		(else_try), 
			####### COMBO BUTTON - ITEM TYPE SELECTOR (DEBUGGING ONLY) #######
			(eq, ":object", "$g_presentation_obj_1"),
			(eq, "$tpe_awards_lock_records", 0),
			(assign, "$tpe_award_group", ":value"),
			(start_presentation, "prsnt_tpe_rewards_window"),

		(else_try),
			##### BUTTON - TAKE VALUABLE ITEM #####
			(troop_slot_eq, TPE_OBJECTS, tpe2_obj_button_take_valuable, ":object"),
			(call_script, "script_tpe_get_most_valuable_record"),
			(call_script, "script_tpe_award_get_record", reg1, 4),
			(assign, ":item_no", reg1),
			(assign, ":imod", reg2),
			(try_begin),
				## FILTER - Invalid Item
				(neg|is_between, ":item_no", TPE_FIRST_ITEM, TPE_LAST_ITEM),
				(display_message, "@Warning - The most valuable item is invalid.", gpu_red),
			(else_try),
				## FILTER - Invalid Item Modifier
				(neg|is_between, ":imod", TPE_IMODS_BEGIN, TPE_IMODS_END),
				(display_message, "@Warning - The most valuable item has an invalid modifier.", gpu_red),
			(else_try),
		...
		(else_try),
			##### BUTTON - TAKE SELECTED ITEM #####
			(troop_slot_eq, TPE_OBJECTS, tpe2_obj_button_take_selected, ":object"),
			(troop_get_slot, ":item_no", TPE_OBJECTS, tpe2_val_selected_item_no),
			(troop_get_slot, ":imod", TPE_OBJECTS, tpe2_val_selected_imod),
			(try_begin),
				## FILTER - Invalid Item
				(neg|is_between, ":item_no", TPE_FIRST_ITEM, TPE_LAST_ITEM),
				(display_message, "@Warning - The selected item is invalid.", gpu_red),
			(else_try),
				## FILTER - Invalid Item Modifier
				(neg|is_between, ":imod", TPE_IMODS_BEGIN, TPE_IMODS_END),
				(display_message, "@Warning - The selected item has an invalid modifier.", gpu_red),
			(else_try),
				(troop_add_item, "trp_player", ":item_no", ":imod"),
				(presentation_set_duration, 0),
				(jump_to_menu, "mnu_town"),
