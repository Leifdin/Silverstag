# General Presentation Utilities (WIP) by Windyplains
# Released --/--/--


###########################################################################################################################
#####                                                MODULE SETTINGS                                                  #####
###########################################################################################################################

# wp_gpu_debug                           = 0   # This turns ON (1) or OFF (0) all of the debug messages.  Set to 2 will enable -very- verbose information.

###########################################################################################################################
#####                                            PRESENTATION DEFINITIONS                                             #####
###########################################################################################################################

# Definintions of text designs
gpu_center_with_outline                    = 1
gpu_center                                 = 2
gpu_left_with_outline                      = 3
gpu_left                                   = 4
gpu_right_with_outline                     = 5
gpu_right                                  = 6

# COLOR CODE DEFINITIONS
gpu_dark_blue  = 68
gpu_blue       = 5887 # 0xFFAAAAFF
gpu_light_blue = 0xFFAAD8FF
gpu_gray       = 3289650
gpu_red        = 0xFFFFAAAA
gpu_yellow     = 0xFFFFFFAA
gpu_pink       = 0xFFFFAAFF
gpu_purple     = 0xFF6AAA89
gpu_black      = 0xFF000000
gpu_white      = 0xFFFFFFFF
gpu_green      = 0xFFAAFFAA
gpu_dark_green = 3072
gpu_brown      = 1508608 # 0xFFAA4800 # WIP
gpu_orange     = 0xFF9A4800 # WIP
gpu_tan        = 0xFF7A4800
gpu_debug      = gpu_light_blue
gpu_error      = gpu_red
gpu_dark_red   = 1441792
#gpu_dark_green = 3584
gpu_dishonorable = 16128      # Dark green
gpu_affiliated   = 101        # Dark blue
gpu_unique       = 10714113   # Yellowish Gold
gpu_owner_only   = gpu_affiliated
# COLOR CODES end

# Modes for the Color Chooser Panel
gpu_ccp_hide       = 0
gpu_ccp_display    = 1

gpu_obj_rgb_label              = 300
gpu_obj_sample_text_background = 301 # Unused
gpu_obj_current_position       = 302
gpu_obj_current_pos_label      = 303
gpu_val_resize_x               = 304
gpu_val_resize_y               = 305
gpu_obj_selected_object_label  = 306
gpu_val_foreground_red         = 307
gpu_val_foreground_green       = 308
gpu_val_foreground_blue        = 309
gpu_obj_red_color_label        = 310
gpu_obj_green_color_label      = 311
gpu_obj_blue_color_label       = 312
gpu_obj_transparency_text      = 313
gpu_obj_transparency_label     = 314
gpu_obj_move_x_label           = 315
gpu_obj_move_y_label           = 316
gpu_obj_resizing_label         = 317
gpu_obj_output_text            = 318
gpu_val_movable_x              = 319
gpu_val_movable_y              = 320
gpu_obj_move_x_text            = 321
gpu_obj_move_y_text            = 322
gpu_obj_movement_label         = 323
gpu_obj_selected_object_text   = 324
gpu_val_transparency           = 325
gpu_obj_red_color_slider       = 326
gpu_obj_green_color_slider     = 327
gpu_obj_blue_color_slider      = 328
gpu_obj_red_color_text         = 329
gpu_obj_green_color_text       = 330
gpu_obj_blue_color_text        = 331
gpu_obj_transparency_slider    = 332
gpu_obj_move_x_slider          = 333
gpu_obj_move_y_slider          = 334
gpu_obj_resize_x_slider        = 335
gpu_obj_resize_y_slider        = 336
gpu_obj_resize_x_text          = 337
gpu_obj_resize_y_text          = 338
gpu_obj_resize_x_label         = 339
gpu_obj_resize_y_label         = 340
gpu_obj_undo_button            = 341
gpu_obj_hide_button            = 342
gpu_obj_show_button            = 343

## BANNER BACKGROUND FIXING ##
GPU_OBJECTS                    = "trp_tpe_presobj"
gpu_obj_main_title             = 1
gpu_val_current_banner         = 2
gpu_obj_current_banner         = 3
gpu_obj_banner_background      = 4
gpu_val_banner_background      = 5
gpu_obj_slider_red             = 6
gpu_obj_slider_green           = 7
gpu_obj_slider_blue            = 8
gpu_val_slider_red             = 9
gpu_val_slider_green           = 10
gpu_val_slider_blue            = 11
gpu_obj_label_red_slider       = 12
gpu_obj_label_green_slider     = 13
gpu_obj_label_blue_slider      = 14
gpu_obj_desc_red_slider        = 15
gpu_obj_desc_green_slider      = 16
gpu_obj_desc_blue_slider       = 17
gpu_obj_button_exit            = 18
gpu_obj_button_next_banner     = 19
gpu_obj_button_prev_banner     = 20
gpu_obj_label_current_banner   = 21
gpu_obj_label_rgb_text         = 22
gpu_obj_label_rgb_value        = 23
gpu_obj_button_update_color    = 24
gpu_obj_button_get_color       = 25

###########################################################################################################################
#####                                       GENERIC HELP GUIDE CONSTANTS                                              #####
###########################################################################################################################
guide_obj_button_exit                   = 1
guide_obj_button_main_topics            = 2
guide_obj_label_main_title              = 3
guide_obj_label_sub_title               = 4
guide_obj_container_credits             = 5
guide_obj_container_info                = 6
guide_obj_topics_begin                  = 7
guide_obj_topics_end                    = 15
guide_val_information_mode              = 24


###########################################################################################################################

dialog_obj_upper_status                 = 24
dialog_obj_upper_portrait               = 25
dialog_obj_upper_name                   = 26
dialog_obj_upper_name_2                 = 27
dialog_obj_upper_updated_time           = 28
dialog_obj_upper_background             = 29
dialog_obj_upper_timer                  = 30
dialog_obj_upper_dialog                 = 31


dialog_obj_lower_status                 = 124
dialog_obj_lower_portrait               = 125
dialog_obj_lower_name                   = 126
dialog_obj_lower_name_2                 = 127
dialog_obj_lower_updated_time           = 128
dialog_obj_lower_background             = 129
dialog_obj_lower_timer                  = 130
dialog_obj_lower_dialog                 = 131