...
# mnu_garrison_switch_modes
	("troop_tree_switch_modes", mnf_scale_picture|mnf_disable_all_keys,
		"I shouldn't see this.  This should automatically be hidden by a new presentation.",
		"none",
		[
			# Wipe clean VAT_OBJECTS
			(try_for_range, ":slot", 0, 400),
				(troop_set_slot, VAT_OBJECTS, ":slot", 0),
			(try_end),
			
			(try_begin),
				####### MODE : TROOP TREE #######
				(eq, "$hub_mode", VAT_MODE_TREE_DISPLAY),
				(change_screen_return),
				(start_presentation, "prsnt_all_troops"),
			
			(else_try),
				####### MODE : TROOP INSPECTION #######
				(eq, "$hub_mode", VAT_MODE_INSPECTION),
				(change_screen_return),
