...
camp_addon = [
      ...
      # ), 
 	  ...
 	  # ("action_prebattle_mod_preferences",[(eq, 1, 0),],
...
def add_pbod_to(objlist, menu_id):
	try:
		...
		for i in range(len(menuoptions)):
			menuOptID = GameMenuOptionWrapper(menuoptions[i]).GetId()
			if not ( #i == attack_i or #not the main attack option, or other options to skip:
			   "siege_request_meeting" == menuOptID or "wait_24_hours" == menuOptID or "talk_to_siege_commander" == menuOptID ##for sieges
			   or "change_commander" in menuOptID or "dplmc_negotiate_with_besieger" == menuOptID or "access_crew" == menuOptID ##for mods that include custom commander, diplomacy, or Floris sea battles
			   ): 
		...
		##new bugfix for siege defense mixups
		if ("siege_started_defender" == menuID): 
			pos = menu.GetOpBlock().FindLineMatching((assign, "$g_siege_first_encounter", 0))
			menu.GetOpBlock().RemoveAt(pos) #remove line from menu opblock, move it to menu options
			for i in range(len(menuoptions)):
				menuOptID = GameMenuOptionWrapper(menuoptions[i]).GetId()
				if menuOptID == "do_orders" or "join" in menuOptID:	
					GameMenuOptionWrapper(menuoptions[i]).GetConsequenceBlock().InsertBefore(0, [(assign, "$g_siege_first_encounter", 0),])
...
def modmerge_game_menus(orig_game_menus, check_duplicates = False):
	try:
		...
		raise
	#Deployment Bugfix/Maintain prior stack order
	try:
		find_i = list_find_first_match_i( orig_game_menus, "battle_debrief" )
		GameMenuWrapper(orig_game_menus[find_i]).GetOpBlock().InsertBefore(0, [(call_script, "script_prebattle_deploy_restore_stack_order"),]),
	except:
		import sys
