	...
	(.5, 0, 0, [(neg|main_hero_fallen)], [
		...
		(try_for_range, ":division", 0, 9),
			...
			(try_begin),
				...
				## WINDYPLAINS+ ##
				# (try_begin),
					# (assign, reg31, reg0), # Size of target team.
					# (assign, reg32, ":slot"), # Slot target team size comes from.
					# (assign, reg33, ":target_team"),
					# (assign, reg34, ":target_division"),
					# (display_message, "@DEBUG (Formations): Target: Team {reg33}, Division {reg34} in slot {reg32} is size {reg31}.", gpu_debug),
				# (try_end),
				## WINDYPLAINS- ##
				(store_add, ":slot", slot_team_d0_target_team, ":division"),
				(team_set_slot, "$fplayer_team_no", ":slot", -1),
